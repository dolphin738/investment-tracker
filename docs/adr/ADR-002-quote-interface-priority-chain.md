> 本文档作为架构决策记录（ADR）。2026-08-13 经用户要求"先写成 ADR 锁定"创建。核心架构决策（方案 X）已收口，3 个运维细节列为待确认项（默认建议，实现前定稿，不视为已锁定决策）。

# 金融数据接口采用"分类级接口优先级链 + 顺序 Fallback + 连续失败告警"（ADR-002）

> 架构师：dolphin738 ｜ 上游输入：多提供方证券行情数据提供方（系统管理页）+ 实时行情消费端（方向 2）｜ 状态：核心决策已收口，待实现
> 变更性质：**行情接口解析模型演进**（全局单一活跃源 → 分类级接口优先级链），触及 `QuoteInterface` 数据模型与 `MarketDataSyncService` 消费端，不改动估值/NAV 计算口径
...
| 项 | 位置（实际代码） | 状态 |
|----|------------------|------|
| 提供方模型 | `backend/app/models/quote_provider.py`：`is_default` / `is_active` / `enabled` | ✅ 全局 provider 级开关 |
...
| 全局活跃源定位 | **降级为粗粒度开关**：`is_active`/`is_default` 不再作为接口解析入口 | 避免与分类级链双模型并存导致语义混乱 |
...
**废弃"全局单一活跃源"作为接口解析入口**，改为按 `category_id` 解析接口优先级链：

> 查询某分类行情时，拉取该 `category_id` 下所有 `enabled=true` 的接口，按 `priority` 升序即优先级，从最高（最小 priority 值）开始顺序调用，有响应即停止。

- `provider.is_active` / `provider.is_default` 退化为"该提供方整体停用"的粗粒度开关（`enabled` 字段已够用，仍保留旗标仅作管理面展示/兼容），**不再参与运行时选源**。
- 影响：方向 2 中 `get_active_provider` 的调用方改为 `MarketDataSyncService.fallback_fetch(category_id, ...)`；并需先解决缺口 G1（证券↔分类映射），否则无法确定"某证券走哪个分类的链"。
...
| 列 | 类型 | 默认 | 用途 |
|----|------|------|------|
| `priority` | Integer, nullable | — | 分类内排序，越小优先级越高；重排事务内批量重排 `UPDATE ... SET priority = CASE id WHEN ... END` |
| `consecutive_failures` | Integer | 0 | 连续无响应计数，原子自增 |
| `alerted` | Boolean | false | 告警去重抢占标志 |
...
| 维度 | 影响 | 处理 |
|------|------|------|
| 现有 `get_active_provider` | 不再作为接口解析入口，降级为管理面兼容/粗粒度开关 | 调用方迁移到 `fallback_fetch(category_id)` |
| 数据模型 | `QuoteInterface` 加 3 列 + 迁移 | `alembic upgrade head` 同步开发/测试库 |
...
| 回退 | 如需回退到全局活跃源模型 | 恢复 `get_active_provider` 调用 + 忽略 `priority` 链；不删列以保留配置 |

---

## 5. 落地范围（分阶段，建议顺序）

1. 数据模型：`priority` / `consecutive_failures` / `alerted` + Alembic 迁移。
...
- `backend/app/models/quote_provider.py` — 提供方模型（is_active/is_default/enabled）
...
- `backend/app/services/quote_provider.py:get_active_provider` — 当前解析链（将被 fallback_fetch 取代）
