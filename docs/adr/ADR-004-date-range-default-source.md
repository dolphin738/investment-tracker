> 本文档已落地·只读，作为架构决策记录（ADR），不再更新（正文保持历史原貌）

> **📝 勘误（v2.8.1 · 2026-08-07）**：本 ADR 正文（§1、§4.1）中「**8 处**范围型位置」为设计时**规划口径**（含 I-06 前瞻性占位）。截至 v2.8，**实际接入为 7 处**——6 处经 `use-default-date-range` hook（XIRR / NAV / 出入金 / 资产记录 / 现金余额历史 / 持仓统一筛选器 I-05），概览趋势栏（`dashboard.tsx`）直接读 `preference.store`（`getPreference('defaultDateRange')`）为第 7 处；「I-06 新增位置」**规划中·尚未接入**。准确表述以主文档 §10.1.8(a) 为准。
