...
| **FIN-D5** | **Newton-Raphson 求解 XIRR**                             | 初值 r₀=0.1，最大迭代 100 次，收敛阈值 \|NPV\| < 1e-7，`rate ≤ -1` 时钳到 -0.999；全同号现金流返回 `null` | 收敛快、实现简单、精度足够；参数已在现网验证                                               |
...
> **XIRR 求解的完整参数与边界处理 —— 初始猜测 r₀ = 0.1、最大迭代 100 次、收敛阈值 |NPV| < 1e-7、`rate ≤ -1 → rate = -0.999` 溢出钳制、导数 `dnpv == 0` 时中断、全同号 / 少于 2 笔现金流返回 `null` —— 一律以【附录 A：XIRR 计算伪代码】为准。**
> 本节与附录 A 若有任何出入，**以附录 A 为权威**。附录 A 与 `FIN-D5`（§2.2）中的 `rate ≤ -1 → -0.999` 溢出钳制**同源同义、已在现网实现，属 F0 冻结内容，不得删改**。
...
| FIN-F0-05 | F0 | Newton-Raphson 求解参数冻结 | r₀=0.1、maxIter=100、tolerance=1e-7、`rate ≤ -1` 钳到 -0.999、全同号返回 null | 见附录 A 伪代码，实现必须逐行对应 |
...
> 另见 PRD.md §5.3（XIRR 迭代参数：r₀=0.1, maxIter=100, tol=1e-7, rate≤-1→-0.999, 同号返回 null）属金融核心，冻结于附录 A。
...
| §5.3 XIRR 迭代参数 | 算法规格 | 承袭（**要点摘要在 §3.7，完整参数以附录 A 为权威**；含 `rate ≤ -1 → -0.999` 溢出钳制，F0 冻结不得删改） | §3.7 / **FIN-D5（§2.2）** / 附录 A | ✅ |
