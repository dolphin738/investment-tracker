...
> **版本**：v3.2.0（Consolidated / 单一权威版）
> **作者**：产品经理 许清楚（Alice）
> **日期**：2026-08-06
> **修订史（完整版）**：每版变更说明 + ID 核算已迁出至 **[docs/PRD-CHANGELOG.md](./PRD-CHANGELOG.md)**（最新为 v3.2.0）。
> **近期变更（v3.2.0）**：**概览融合改版收口** —— 出入金页【A】总资产概览块整体迁移至概览页：指标卡 6→8（资产构成 4 + 收益表现 4，`DASH-P0-01`），新增「趋势分析」分区（日期筛选 + 总资产走势图 hero + 四宫格，`DASH-P0-02` / §7.4），走势图卡头为**单一 `/snapshots` 入口**；出入金页仅保留现金余额【B】与流水列表【C】。**无需求 ID 增减**（合计 240 / 枚举行 222 / 遗漏 0 不变）。
> **语言**：中文（简体）
...
| HOLD-B-P0-11 | P0 | 筛选与历史日期切换 | 顶部组合选择器（复用 `portfolio-selector`）+ 标的类型多选筛选 + **持仓日期选择（可选任意历史日期，默认今日）** + **「显示已清仓」开关** | 1) 切换组合后列表与合计同步刷新；2) 类型筛选支持多选（股票/基金/债券/其他，ETF 归入基金 `FUND`，全不选 = 全部）；3) 日期可选**首笔流水日之后的任意日期**（方案 B 下任意日期均可精确推导，不再限制"仅有数据的日期"），下限 = 首个交易日、上限 = 今日；4) **三组筛选写入 URL query**（`date`/`closed`/`types`/`sec`，等于默认值不写入 URL，刷新/分享后保持）；「显示已清仓」初值 = `UserPreference.showLiquidated`，**URL 参数优先级高于偏好** |
