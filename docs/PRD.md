...
>
> 🔍 每一条源需求/约束/决策的去向，见 **§13 新旧覆盖对照表**（遗漏数 = 0）。
...
| 附录 | A XIRR 伪代码 / B 净值伪代码 / C 持仓推导伪代码 / D 数据模型汇总 / E 接口清单 |
...
| **④ 已废止** | 演化噪音：五代版本对照 / 变更摘要 / 作废声明 / `PRD-incremental-v2.0.md` 全文 | — | **丢弃**。但其中的**有效决策与需求 ID 已全部迁入本文档**（见 §13） |
...
- **零重复**：`ChangeEmailDialog` / `ChangePasswordDialog` / `EditProfileDialog` 与头像上传/URL 设置**全部归属设置页**，账户模块**不引用、不挂载**这些 Dialog
...
- **唯一修改入口**：**修改邮箱 / 修改密码 / 编辑资料（昵称·手机·简介）/ 头像修改（本地上传 + URL 设置）** 四类动作全部且仅在本页账户区（`SET-P0-01`）
...
> **ID 稳定性承诺**：本章所有 ID 为**稳定 ID**，后续迭代只增不改。源 PRD 中的旧 ID 与本章 ID 的映射关系见 §13。
...
| 编号        | 优先级 | 功能点          | 描述                                                                                         | 验收标准                                                                                                                                                                                                 |
| --------- | --- | ------------ | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SET-P0-01 | P0  | 账户资料与安全设置（**全站唯一修改入口**）        | 设置页账户区承载**全站唯一**的账户修改能力：**修改邮箱 / 修改密码 / 编辑资料（昵称·手机·简介）/ 头像修改（本地上传 + URL 设置）**；区块顶部保留「头像 + 昵称 + 邮箱」一行摘要 + 「前往账户中心 →」链接                             | 1) 四类修改动作**全部且仅**在本区，复用现有 `ChangeEmailDialog` / `ChangePasswordDialog` / `EditProfileDialog`，**不要求改动这些组件的内部实现**；2) 头像修改支持**本地上传**（已有）与 **URL 设置**（输入图片 URL 即可用作头像），**两种方式均归本区**；3) **账户页 `/account` 不再提供任何修改入口**，本区为全站唯一入口，不得存在第二处账户资料修改入口；4) 手机号与简介的详细只读展示迁至账户页，本区只留摘要，与 `/account` 无信息重复；5) 修改成功后 toast 提示并刷新 profile 缓存                                                                                                                                    |
...
| 区块 | 承载需求 | 说明 |
|------|---------|------|
| **账户与会话区** | `SET-P0-01`（账户资料与安全设置 · 全站唯一修改入口）、**`SET-P0-09`（退出登录）** | 区块顶部为「头像 + 昵称 + 邮箱」摘要 + 「前往账户中心 →」；四类修改动作在此；**退出登录**置于本区底部 |
...
> 🔴 **遗漏 = 0，达成交付标准。** 三份来源共 **235 个**需求 / 约束 / 决策 / 风险 / 问题 / 规格条目全部在本 PRD 落位，另加 v3.1.0 新增派生 ID **2 个**、v3.1.1 新增派生 ID **2 个**，合计 **239**（实际枚举表格行 **221 行**，其中 7 行将多个用户故事 / 问题 ID 归并为一行，如 `US-D1~US-D5`、`Q-01~Q-14`）。含明确改判 / 废止并记录的 D-01、HOLD-P0-02/05/07、D-04、D-05、C-09、TXN 模块并入等。
>
> 📌 **v3.0.1 修订说明（相对 v3.0 首版 232 项）**：本次新增 3 个章节级条目的显式核对行 —— ① `PRD.md §5.3` XIRR 迭代参数（原仅隐含于 §3.7 / FIN-D5 / 附录 A，本次显式登记）；② `PRD.md §6.3` 双端交互差异表（**首版遗漏，本次补入 §9.4.1**）；③ `v2.3 §9.3` 强制回归测试 5 项（**首版被打散，本次提升为独立章节 §10 `REG-01`~`REG-05`**）。故 ID 数由 232 → 235，枚举行由 213 → 216。**首版 232 项的覆盖结论不变，遗漏数仍为 0。**
>
> 📌 **v3.1.0 修订说明（相对 v3.0.1 的 235 项）**：因 `ACC-P1-01`（数据统计卡）与 `SET-P1-01`（外观主题）升级为 P0，依 §6.0 **ID 稳定性承诺（只增不改、旧 ID 不复用）**，采用「**新增 ID + 旧 ID 保留为升级记录**」处理，派生 **`ACC-P0-06`** / **`SET-P0-08`** 两个新稳定 ID；旧 `ACC-P1-01` / `SET-P1-01` 行**保留不删除**，仅改写为升级记录并把「新 PRD 落位」指向新 ID。故 ID 数由 235 → 237，枚举行由 216 → 218。**源 PRD 三份的 235 项覆盖结论不变，遗漏数仍为 0。**
>
> 📌 **v3.1.1 修订说明（相对 v3.1.0 的 237 项）**：因**退出登录**（原属 `ACC-P0-05` 会话动作）与**注销账户**（原 `ACC-P1-03`）**自账户页迁入设置页**，依 §6.0 **ID 稳定性承诺（只增不改、旧 ID 不复用、不删除）**，采用「**新增 ID + 旧 ID 保留为迁移记录**」处理，派生 **`SET-P0-09`**（退出登录，P0）/ **`SET-P1-06`**（注销账户，P1）两个新稳定 ID；旧 `ACC-P1-03` 行**保留不删除**，仅改写为迁移记录并把「新 PRD 落位」指向 `SET-P1-06`；`ACC-P0-05` 保留原 ID 并改名「**跳转区**」（功能收敛为纯跳转）。故 ID 数由 237 → **239**，枚举行由 218 → **220**。**源 PRD 三份的 235 项覆盖结论不变，遗漏数仍为 0。**

> 📌 **v3.1.2 修订说明（相对 v3.1.1 的 239 项）**：**纯枚举补登，无 ID 增减**。`SET-P0-07`（软提示开关，P0）需求本体自 v3.0 起已存在（§6.9、§6.9.1 `UserPreference` 字段、§6.9.2 偏好与外观区），亦已计入 §13 合计 239 与覆盖结论，唯独**漏登于 §13 设置模块枚举表**，本次补该展示行。故 **ID 数仍为 239（无增减）**，**枚举行由 220 → 221**（补登 1 行）。合计 239 / 遗漏 0 不变，F0 冻结内容一字未改。

> 📌 **v3.1.3 修订说明（D-1 钳制修正 · F0 冻结内容的唯一授权例外）**：**无需求变更、无 ID 增减**，合计仍为 **239 / 枚举行 221 / 遗漏 0**。本次仅同步 XIRR 溢出钳制的**触发阈值**：`rate ≤ -1` → **`rate ≤ -0.999`**，**钳定值 −0.999 保持不变**（钳完仍为 `-0.999`，两者差别仅在触发边界）。依据：现网代码 `packages/finance-core/src/xirr.ts` 中 `const MIN_RATE = -0.999;` 且 `if (rate <= MIN_RATE)`，即代码采用**更宽、更安全**的下钳边界——比 PRD 原 `≤ -1` 多钳住 **[−100%, −99.9%]** 区间，避免该段极端负收益率裸放进 `(1+r)^t` 幂运算。经**用户明确授权**，本次**例外性地违反 `FIN-F0-05`「实现必须逐行对应 PRD」规则**，改为**将 PRD 反向对齐至代码**。此为 F0 冻结文本的**唯一授权例外**：Newton-Raphson 其余参数（r₀=0.1 / maxIter=100 / tolerance=1e-7 / `dnpv == 0` 中断 / 同日合并 / 全同号返回 `null`）、§3 金融口径、其余 `FIN-*`、附录 A 除该行外的全部行、附录 B（`calculateDailyNav` 与期末口径注释）**一字未改**。

> 📌 **v3.1.4 修订说明（配色口径统一 · 纯展示层修订）**：**无需求变更、无 ID 增减**，合计仍为 **239 / 枚举行 221 / 遗漏 0**。本次仅统一「涨跌 / 盈亏 / 金额增减」的正负语义配色，全局采用 **A 股惯例：正值 = 红色、负值 = 绿色**，消除 PRD 内部反向表述。具体改动三处：① **`ANL-P0-06`**（§6.7 分析页）验收 1 由「正收益**绿色**、负收益**红色**」修正为「**正收益红色、负收益绿色**（A 股涨跌色，遵循 §9.5）」；② **§7.6 净值分析页线框图注释**（每日净值明细表下方）由「正收益绿色 / 负收益红色」修正为「**正收益红色 / 负收益绿色**」；③ **新增 §9.5「展示规范 —— 全局涨跌配色约定（A 股惯例 · 正红负绿）」**，明确适用范围（图表 / 明细表 / 卡片 / 徽标）、色值必须走主题变量、与正负语义无关的着色不受约束、**后续新增需求不得出现反向配色**，并在 §2.5 全局约束表后补一条交叉引用。**§9.5 为无 ID 规范章节**（体例与 §9.1 精度 / §9.2 性能 / §9.3 安全性 / §9.4 多端一致性 一致），**不新增需求 ID，不触发 §13 覆盖表任何计数变化**。原文即符合正红负绿的 `FLOW-P0-07`、`DASH-P0-05`、`DASH-P1-05`、`HOLD-B-P0-04` **一字未改**。**裁决来源：用户（产品负责人）拍板 ——「PRD 全部改成 A 股正红负绿」**，架构评审阶段由架构师标注的「ANL-P0-06 配色矛盾，待 PM 澄清」至此闭环。**F0 冻结内容（§3 金融口径、`FIN-*`、XIRR / 净值 / 份额 / 精度、附录 A、附录 B）一字未改**，本次为纯展示层修订。
