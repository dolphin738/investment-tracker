...
> **版本**：v3.3.0（Consolidated / 单一权威版）
> **作者**：产品经理 许清楚（Alice）
> **日期**：2026-08-07
> **修订史（完整版）**：每版变更说明 + ID 核算已迁出至 **[docs/PRD-CHANGELOG.md](./PRD-CHANGELOG.md)**（最新为 v3.3.0）。
> **近期变更（v3.3.0）**：**2026-08-07 增量 PRD（I-01 ~ I-06）正式并入主文档，以实际交付形态为准**（已实现 + QA 两轮全绿 1080/1080，commit `79f5d12` / `7f84906` / `6a36b99`）—— ① 买卖流水**编辑态与录入态共用同一表单**（`HOLD-B-P0-02` / `HOLD-B-P0-07`）；② 分红**所得税可选、净额 = 金额 − 所得税由后端统一计算**，编辑允许改 `type`（`HOLD-B-P0-10`）；③ 费用记录**采用「展示层聚合 + 明细行保留」**（🔴 **非**数据库物理合并）+ 新增 `scenario`（买入时/卖出时）+ `PATCH /fees/:id`（`HOLD-B-P0-10`）；④ **默认日期范围取值域扩为 7 项**、单一真相源 `QUICK_RANGE_OPTIONS`、8 处全局生效（`SET-P0-02` / §6.9.1 / `DASH-P0-02`）；⑤ 持仓页**三板块共享统一筛选器**，as-of 与日期范围**口径独立**（`HOLD-B-P0-11`）；⑥ 新增 **`SYS-P1-03`** 日期选择器统一规范（范围型 100% 提供快捷范围）。另新增 **`SET-P2-05`**（费用记录导出含场景列，遗留登记为 P2）。**新增 2 个需求 ID**（合计 242 / 枚举行 224 / 遗漏 0）。**F0 冻结内容（§3 金融口径 / `FIN-*` / 附录 A / 附录 B）一字未改**。
> **语言**：中文（简体）
...
| 后端技术栈 | NestJS + Prisma + PostgreSQL 16；共享金融算法层 `finance-core`（零依赖纯金融算法库，XIRR / 净值等核心算法独立于此包） |
...
| APP 技术栈 | ~~HarmonyOS（ArkTS + ArkUI）~~ **已移除**（`packages/harmonyos` 包已删除，仅保留 Web 端） |
| 仓库结构 | pnpm monorepo（`packages/backend` / `web` / `shared` / `finance-core`） |
| 本阶段交付范围 | **仅 `backend` + `web` + `shared` + `finance-core`**（`packages/harmonyos` 已移除） |
| 代码管理 | Git（monorepo） |
...
6. 前端使用 Web 版本（原计划含 HarmonyOS APP，该端已移除），使用 Git 管理代码
...
| ❌ 社交 / 社区功能 | 超出产品定位 |
| ❌ 税务计算与合规报表 | 超出产品定位 |
| ❌ ~~HarmonyOS 端适配~~ **已移除** | `packages/harmonyos` 包已删除，仅保留 Web 端 |
...
| 页面归属 | 出入金管理页（`/cashflows`，`/transactions` 保留 301 重定向），并**并入**现金余额录入区；原【A】总资产展示区（当前值 + 近期走势 + 手工记录标记）已随 **v3.2.0 概览融合改版整体迁移至概览页**（见 §6.6 / §7.4），本页不再展示总资产；历史总资产记录统一于 `/snapshots` 浏览与管理 |
...
| `DividendRecord`（分红记录） | 日期 / 标的 / 金额 / **所得税 `tax`（可选，空视为 0，≥ 0）** / 类型（现金分红·红利再投）/ 备注。**不参与收益计算**（D-02 有效）。红利再投不录入（无现金进出）。🆕 **净额 `netAmount` = 金额 − 所得税**，**由后端统一计算并随响应返回、不落库**（延续 K-2，前端不二次计算） |
| `FeeRecord`（费用记录） | 日期 / 标的 / 金额 / 费用类型（佣金·印花税·其他）/ **场景 `scenario`（`BUY` 买入时 / `SELL` 卖出时，非空）** / 关联流水 ID（可选）/ 备注。**不参与收益计算**（D-03 有效）。🆕 **合并只发生在展示层**：`GET /fees?grouped=1` 按合并键 `(portfolioId, securityId, date, scenario, type)` 聚合出合计行，**底层明细行原样保留**（不物理合并、无数据库唯一约束），详见 `HOLD-B-P0-10` |
...
写入语义（upsert，唯一权威口径）：
  ...
  deleteRecord(portfolioId, D):
      ...
      // 随后由 recalculation.service 重算 [D, today] 的 daily_nav / daily_xirr
...
> 🔀 **v3.2.0 概览融合改版**：本页已承接出入金页【A】总资产概览能力 —— ① 指标卡扩为 **8 张**、分两组：**资产构成**（当前总资产 / 持仓市值 / 现金余额 / 净投入）+ **收益表现**（累计收益率 / 当年收益率 / 年化 XIRR / 累计净值），见 §6.6 `DASH-P0-01`；② 新增「趋势分析」分区：日期筛选栏（快捷区间 + 自定义起止，复用 URL `custom/from/to`）→ 总资产走势图（hero，含手工记录散点，卡头**单一 `/snapshots` 入口**）→ 四宫格，见 §6.6 `DASH-P0-02` / §7.4。
...
- **注销的反向操作不在本页**：注销后账户进入 **30 天冷静期**，恢复入口在**登录页**（自助恢复，`SYS-P1-02`，契约见 §6.10.1、草图见 §7.10），**不在设置页** —— 用户此时已无有效会话，进不来设置页。本页的注销确认文案须告知"30 天内可在登录页用原邮箱 + 密码自助恢复"
- 把当前 3 个 `disabled` 的数据管理占位按钮真正落地
- 偏好设置从 `localStorage` 升级为**服务端持久化**（`UserPreference`），实现多端/多浏览器一致
- **偏好即全局默认（v3.3.0）**：「默认日期范围」的**取值域 = 全站「快捷范围」同一组 7 项**（`1w/1m/3m/6m/1y/ytd/all`），来自**单一真相源** `QUICK_RANGE_OPTIONS`；设置页**不得**再维护第二份选项数组。该偏好在全站 **8 处**范围型日期位置生效，优先级 **URL 参数 > 用户偏好 > 系统默认 `1y`**（详见 `SET-P0-02` / §6.9.1 / `SYS-P1-03`）
...
> **一致性约束**：§7.8 设置页草图的「① 账户与会话区」**不得**再出现独立的「头像 URL」输入框；头像 URL 的唯一录入位置为本卡片。
...
| FLOW-P0-03 | ~~P0~~ | 总资产区块（**已迁移至概览页**） | 原「出入金页顶部展示总资产概览（`totalAsset` 当日记录值 + 精简历史曲线 + 管理历史记录入口）」能力已随 **v3.2.0 概览融合改版整体迁移至概览页** —— 指标卡见 §6.6 `DASH-P0-01`，走势图 / 日期筛选 / 历史入口见 §6.6 `DASH-P0-02` 与 §7.4；**出入金页不再展示总资产**。本 ID 依 §6.0 ID 稳定性承诺**保留为迁移记录**，不复用、不删除 | — |
...
| FLOW-P1-01 | P1 | CSV/Excel 导入 / 导出 | 模板含新字段集（**无证券明细列**）；导出当前筛选结果，格式 `csv` 或 `xlsx` | 1) CSV 为 UTF-8 **带 BOM**，Excel 中文不乱码（xlsx 由后端 `xlsx` 库生成，同构）；2) 导入前预览前 10 行 + 显示识别总行数（**预览阶段不落库**）；3) 行级报错定位到行号 + 字段 + 原因，错误可导出；4) 可选「跳过错误行」/「全部回滚」，默认全部回滚；5) 导入完成后**一次性触发重算**（单 Prisma 事务写入，事务提交后仅调用一次 `recalculateNavRange(pid, minDate)`，自最小导入日期至今日，而非逐笔）；6) 导出行数与筛选结果一致 |
...
| HOLD-B-P0-02 | P0 | 买卖明细录入 / **编辑**表单（**录入态与编辑态完全统一**） | 标的（下拉 + 内联新建）/ 方向 / 日期 / 数量 / 成交额 / **费用三框（佣金·印花税·其他）** / 备注；自动显示**含费单价**只读预览。🆕 **v3.3.0（原增量 I-01）**：**录入与编辑共用同一个 `SecurityTradeForm` 组件、同一 schema、同一字段顺序与同一布局**，`isEdit` 仅决定初始值与提交目标（`POST` vs `PATCH`），**不再分叉 UI**（旧「编辑态不展示费用三框、后端 update 忽略 `fee`」的 U-4 行为已反转） | 1) 录入态与编辑态**字段集合、顺序、校验、预览完全一致**：方向 → 日期 → 标的 → 数量 → 成交额 → 费用三框 → 含费单价预览（只读）→ 备注；2) **编辑态同样展示费用三框**，按 `transactionId = trade.id` 的 `FeeRecord` **按费用类型拆分回填**，未关联则为空；3) 含费单价预览实时刷新：`price = (成交额 ± 费用合计) / 数量`（买入 `−`、卖出 `+`），6 位小数收敛；4) 校验一致：`quantity > 0`、`price > 0`、`fee ≥ 0`、日期不可未来、卖出数量硬校验（`HOLD-B-P0-08`）、卖出时 `费用合计 ≤ 成交额`；5) **保存时费用可被更新**：后端 `PATCH /security-trades/:id` 支持 `fee` 落库；前端按统一口径提交 `fee: 0`（含费单价入 `price`），并对该笔 `transactionId` 关联的 `FeeRecord` 执行**删旧插新重建**（改金额 → 更新、由无到有 → 新建、由有到无 → 删除），`scenario` 自动取该笔流水方向（`BUY_SEC→BUY` / `SELL_SEC→SELL`）；6) **存量旧口径兼容**：编辑态检测到 `trade.fee ≠ 0` 且无关联 FeeRecord 时，费用三框回填「其他」= `trade.fee` 并展示 amber 提示「旧口径费用将并入含费单价」，保存后成本守恒；7) 保存后持仓列表与总资产区块即时刷新；8) 保存后**自当日起的自动总资产记录被重建**（手工记录日期跳过）；9) 保存后 `CASH-P0-05` 现金余额软提示行为不变 |
...
| HOLD-B-P0-07 | P0 | 买卖流水列表与编辑删除 | 按标的/日期/方向筛选，可编辑删除。🆕 **v3.3.0**：筛选能力**上移至持仓页统一筛选器**（`HOLD-B-P0-11`），本板块不再自带独立筛选栏；「编辑」入口打开的弹窗即 `HOLD-B-P0-02` 的同一表单 | 1) 编辑删除后从该日起级联重算（沿用 `recalculation.service`）；2) toast 提示影响范围；3) 🆕 编辑弹窗与录入弹窗**为同一组件**，用户无需重新学习操作；4) 🆕 编辑历史日期流水 → 从该日起级联重算并 toast 提示影响范围（含跳过的手工记录数） |
...
| HOLD-B-P0-10 | P0 | 分红 / 费用独立记录（🆕 **含分红所得税净额** + **费用场景与合并展示**） | `DividendRecord`（日期/标的/金额/**所得税 `tax` 可选**/类型（现金分红·红利再投）/备注）与 `FeeRecord`（日期/标的/金额/费用类型（佣金·印花税·其他）/**场景 `scenario`：`BUY` 买入时 · `SELL` 卖出时**/关联流水 ID 可选/备注）。**两表均不参与 XIRR 与净值计算**（D-02 / D-03）。✅ **后端 NestJS 模块（`DividendModule` / `FeeModule`）已于阶段 C 恢复**（Q-1 决策 A）。🆕 **v3.3.0（原增量 I-02 + I-03）**：分红净额口径落定 + 编辑分红 400 报错修复；费用**按合并键在展示层聚合**并标注买入时/卖出时；新增 `PATCH /fees/:id` 支持修正 | 1) 两表 CRUD + 权限隔离（`user_id` 过滤，经 `portfolio.userId` 校验，另含标的归属二级校验）；2) 可在持仓模块按标的查看累计分红与累计费用（持仓页【E】「分红/费用」Tab）；3) **红利再投不录入**（无现金进出，其价值自然体现在持仓市值上升中）；4) 集成验证：分红/费用数据**不污染** `daily_nav` / `daily_xirr`（回归：三张收益表 `daily_nav` / `daily_xirr` / `cash_flows` 在分红费用增删改时零触碰）；5) 🆕 **分红所得税与净额**：`tax` **可选**（不传 / 留空 = 0，`≥ 0`），**净额 = 金额 − 所得税**，**由后端 `toResponse()` 统一计算**并随响应返回、**不落库**（Prisma 零新增列，维持 `amount` + `tax` 两列，延续 K-2），前端只展示（2 位小数）、不自行二次计算；6) 🆕 **三场景全部正确**：① 录入时填税（320 / 60 → 净额 260）② 录入时不填税（320 → 净额 320）③ 编辑时后补税（补 60 → 净额 260）；7) 🆕 **编辑分红允许携带并修改 `type`**（`CASH` ⇄ `STOCK_DIVIDEND`）：`UpdateDividendRecordDto` 声明 `type?`（`@IsOptional @IsEnum`）且 service 落库，**不得再出现「property type should not exist」400**（根因：全局 `ValidationPipe` 的 `whitelist + forbidNonWhitelisted` 遇 DTO 未声明字段即拒）；8) 🆕 **分红校验与提示**：`amount > 0`「分红金额必须大于 0」；`tax ≥ 0`「所得税不能为负」；`tax = amount` → 净额 0 **允许**；**`tax > amount` → 400「净额不能为负」**（前端 zod 同步拦截，规则与后端 DTO 一致）；9) 🆕 **费用场景 `scenario` 非空**：录入表单提供**必选**「场景」选择器（买入时 / 卖出时）；由买卖流水联动创建时按 `SecurityTrade.side` 自动推断（`BUY_SEC→BUY` / `SELL_SEC→SELL`）；无法推断时回退默认 `BUY`；列表以**徽标**展示「买入时 / 卖出时」；10) 🔴 🆕 **费用合并 = 展示层聚合，非物理合并**：`GET /fees?grouped=1` 按**合并键 `(portfolioId, securityId, date, scenario, type)`** 聚合返回合计行（含 `amount` = Σ 金额、`count` = 组成笔数、`transactionIds[]`），**底层明细行原样保留、各自持有自己的 `transactionId`**；**不建数据库唯一约束、不做累加 upsert**（架构裁决 Q-8：物理合并会破坏 `HOLD-B-P0-02` 编辑态「按 `transactionId` 精确重建」语义，且金融数据需保留明细可审计）；11) 🆕 **合并行为断言**：同合并键多笔 → 1 行合计；同日同标的同场景但**类型不同** → 2 行；同日同标的同类型但**场景不同** → 2 行；修改/删除任一组成笔后重查即正确重算（`groupByMergeKey` 为纯函数）；合并行在 UI 显示组成笔数（如「2 笔」），编辑作用于代表明细行、合并行删除禁用；12) 🆕 **费用修正入口**：新增 `PATCH /api/portfolios/:id/fees/:feeId`（双闸鉴权，可改 `securityId/date/amount/type/scenario/note`），用于修正 migration 回填后默认 `BUY` 的存量记录；13) 🆕 **存量回填策略**：能按 `transactionId` 推断的取 `SecurityTrade.side`，无法推断的默认 `BUY`（不静默修正，由用户按验收 12 手动改）；14) ⚠️ **费用导出的 `scenario` 列不在本条范围内** —— 当前导出模块无 FEES 类别，已另行登记为 **`SET-P2-05`** |
| HOLD-B-P0-11 | P0 | **持仓页统一筛选器**（原「筛选与历史日期切换」，v3.3.0 重构） | 顶部组合选择器（复用 `portfolio-selector`）+ 🆕 **一个统一筛选器承载全部筛选维度，由【B】持仓列表 /【C】买卖明细 /【E】分红费用 三板块共享**。原「持仓日期卡片」**原地升级**为统一筛选器容器（`HoldingsToolbar`），标题「统一筛选器」，卡片内含：**快捷范围 7 项 + 自定义起止** · **持仓日期（as-of）单点** · **证券多选** · **场景（买入/卖出/全部）** · **持仓专属折叠区（类型多选 + 显示已清仓）**。🔴 **as-of 与日期范围为两套独立口径，互不换算**（架构裁决 Q-6） | 1) 切换组合后列表与合计同步刷新，筛选重置为默认；2) 类型筛选支持多选（股票/基金/债券/其他，ETF 归入基金 `FUND`，全不选 = 全部）；3) **持仓日期（as-of）**可选**首笔流水日之后的任意日期**（方案 B 下任意日期均可精确推导），下限 = 首个交易日、上限 = 今日、默认今日 —— **原持仓日期能力零丢失**；4) **筛选状态全部写入 URL query**：`date`（as-of）/ `closed` / `types` / `sec`（🆕 升级为逗号分隔多值）/ 🆕 `range` / 🆕 `from` / 🆕 `to` / 🆕 `scenario`；**等于默认值不写入 URL**，刷新 / 分享后原样还原；5) 「显示已清仓」初值 = `UserPreference.showLiquidated`、日期范围初值 = `UserPreference.defaultDateRange`（`SET-P0-02`），**URL 参数优先级均高于偏好**；🔴 **用户手动操作后不得被偏好对齐逻辑弹回**（偏好对齐只在「偏好异步到达 + URL 无对应参数 + 用户尚未主动操作」时执行一次）；6) 🆕 **三板块联动口径（各板块只响应"适用维度"）**：**日期范围** → 只驱动【C】买卖明细与【E】分红费用（区间过滤），**持仓板块不受影响**；**as-of** → 只驱动【B】持仓（精确推导），买卖明细/分红费用不受影响；**证券多选** → 三板块同时过滤；**场景** → 【C】按方向过滤（`BUY→BUY_SEC` / `SELL→SELL_SEC`）、【E】按 `FeeRecord.scenario` 过滤（分红无场景维度、不适用），**【B】持仓不适用（置灰/隐藏）**；**类型多选 / 显示已清仓** → 持仓板块专属；7) UI 必须注明口径提示：「持仓板块以持仓日期为准，买卖明细 / 分红费用以日期范围为准」；8) 三板块各自空状态四态齐全（C-06），汇总条（`HOLD-B-P0-06`）与累计分红/费用随筛选动态变化；9) 卡片内日期范围**必须提供 7 项快捷范围**（`SYS-P1-03`）；10) 证券多选与类型多选叠加为 **AND** 关系；无任何流水时筛选器仍可用，三板块显示空状态 + 清除筛选按钮 |
| HOLD-B-P1-01 | P1 | 买卖流水 CSV/Excel 导入 | 模板 + 行级报错 + 一次性重算 | 1) 提供模板下载（csv/xlsx 双格式）；2) 逐行校验并报错到行号 + 字段 + 原因；3) 导入后一次性触发重算（单事务 + 单次 `recalculateNavRange`） |
...
| SNAP-P0-02      | P0     | 总资产卡数据契约                     | 总资产卡（**概览页**，见 §6.6 `DASH-P0-01` / §7.4）数据契约；原「出入金页总资产区块（`FLOW-P0-03`）」已迁移为概览页能力 | 返回 `{ totalAsset, marketValue, cashBalance, priceAsOf, cashAsOf, date, source, valuationFlag, recordedAt, derivedTotalAsset }`；`source` 为 `DERIVED` 或 `MANUAL`；当 `source='MANUAL'` 时 `derivedTotalAsset` 由 **`computeDerived(date)` 实时计算**得出（**不是**从库中另一行读取），当 `source='DERIVED'` 时该字段**等于 `totalAsset`**（该行本就是系统派生结果，不重复计算）；计算失败/数据缺失时该字段为 `null` 且接口**照常 200**（派生值是增强信息，不是核心数据）                                                                                                                                                         |
...
| 编号 | 优先级 | 功能点 | 描述 | 验收标准 |
|------|--------|--------|------|---------|
| DASH-P0-01 | P0 | 核心指标卡片补全 | **卡片集固定为 8 个，分两组**（v3.2.0 融合自出入金页【A】总资产概览）：**资产构成**（当前总资产、持仓市值、现金余额、净投入）+ **收益表现**（累计收益率、当年收益率、年化 XIRR（累计）、累计净值）；已去重原独立的「当前总资产」卡（出入金页 3 卡中的「当前总资产」与概览页原 6 卡重复，融合后只保留 1 张）。原写死为 `-` 的「最大回撤」卡片**不在此列**，待 `DASH-P1-02` 实现后再上架 | 1) 8 张卡片全部有真实数据，**不得保留 `-` 占位**；2) 无数据时显示"暂无数据"+ 引导录入，不报错；3) 金额千分位 + 2 位小数，收益率百分比 2 位，净值 4 位；4) 每张卡片副标题标注数据截止日期；5) 🔴 **「当前总资产」数据源必须是 `AssetSnapshot` 的当日/最新唯一记录**，**不得**在概览侧自行拼装 `Σ(quantity×price) + cashBalance`（C-08′）；6) 若最新记录 `source='MANUAL'`，卡片带 `✋手工` 徽标；7) **总资产走势口径**（`v3.2.0`）：`totalAsset = cumulativeNav × shares`，任一为 `null` 则该点丢弃 —— **仅 `aggregation=LAST` 成立**（若引入 AVG 聚合，`avg(nav) × avg(shares) ≠ avg(nav × shares)`，须改为后端直出总资产序列） |
| DASH-P0-02 | P0 | 时间维度切换器 + 走势图日期筛选 | 顶部提供 `日/周/月/年` Tab + 日期范围选择器（快捷项：**近一周 / 近 1 月 / 近 3 月 / 近 6 月 / 近 1 年 / 今年 / 全部（7 项，决策 Q-6 乙）+ 自定义起止**，复用 URL `custom/from/to`）。**v3.2.0**：总资产走势图（hero）支持与净值 / XIRR 相同的日期筛选，不再固定近 30 天 | 1) 复用 `features/query/dimension-switcher.tsx`，快捷范围来自**单一真相源 `QUICK_RANGE_OPTIONS`**（7 项）；2) **默认值「按月 + 近 1 年」**，可被用户偏好覆盖（`SET-P0-02`），偏好为空时回落到「月 + 近 1 年」；🆕 **v3.3.0 优先级钉死：URL 参数 > `UserPreference.defaultDateRange` > 系统默认 `1y`**（偏好异步到达且 URL 无 `range`/`from`/`to` 时对齐一次，用户已主动操作则不再对齐）；3) 切换后**总资产走势 / 净值趋势 / XIRR 趋势三张时序图同步刷新**；4) **维度与范围写入 URL query**（key：`g`/`range`/`from`/`to`，等于默认值不写入 URL，刷新/分享后保持；手改起止 → `range=custom&from=…&to=…`）；5) 单次查询响应 < 500ms；6) **总资产走势图卡头提供单一 `/snapshots` 入口**（「查看全部历史」） |
...
| DASH-P1-01 | P1 | 多组合表现对比 | 表格列出全部组合：名称 / 最新总资产 / 累计净值 / 累计收益率 / 当年收益率 / 年化 XIRR / 最后更新日 | 1) 后端新增批量汇总接口 `GET /api/portfolios/comparison`（一次查询返回全部组合摘要，避免 N+1）；2) 点击行切换当前组合；3) 支持按任意列排序；4) 组合数 ≤ 20 时响应 < 800ms |
...
| DASH-P1-03 | P1 | 数据新鲜度提醒 | **最新现价 asOf 或现金余额 asOf** 距今 > N 个自然日时，页面顶部显示提示条 | 1) 判定**全部在后端**完成（概览接口返回 `freshness` 聚合对象，前端只渲染）：行情维度 = 当前持仓标的中**最落后的那只** `MIN(MAX(SecurityPrice.asOf) per held security)`（只要一只持仓标的无行情记录即视为最陈旧）；现金维度 = `MAX(CashBalance.asOf)`；滞后天数 = asOf → 今天（应用时区 UTC+8）的自然日差；`isStale` = 任一维度滞后 > `staleDays`；2) 提示条按原因（行情/现金）分别渲染文案，提供三个按钮：**「去更新行情」**（跳持仓页）/ **「去更新现金余额」**（跳 `/cashflows`）/ **「本次会话不再提示」**（sessionStorage，按组合隔离，关标签页失效）；3) 阈值天数可在设置中调整（默认 3，`UserPreference.staleDays`，1~30）；4) `isStale === false` 时**完全不渲染**（不占位、无布局跳动）；⚠️ 判定口径已按方案 B 更新（**不再是"快照未更新"**，也**不再用快照 `latestDate`**） |
...
| ACC-P0-02 | P0     | 用户信息卡（只读）         | 展示：头像、昵称、邮箱、手机（脱敏 `138****8000`）、个人简介、注册时间                                            | 1) 复用 `UserAvatar` 组件与 `GET /api/auth/profile`；2) 手机号脱敏规则与设置页一致；3) 未填写字段显示 `-`；4) **纯只读卡：卡内不得出现任何修改控件**——无"编辑资料"按钮、无头像上传、无头像 URL 设置，头像仅作展示不可点击；5) 卡片内提供「前往设置修改 →」链接（实现位于卡片底部，**全页唯一**），跳转 `/settings` 账户区（`SET-P0-01`）完成邮箱/密码/资料/头像修改 |
| ACC-P0-03 | P0     | 资产全景卡             | 展示：组合总数、**合计总资产**（各组合最新记录求和）、合计净投入、合计浮动盈亏                                             | 1) 数据源为 `GET /api/portfolios/comparison`；2) **明确不做跨组合合计 XIRR / 合计净值**（口径需先评审，`Q-07`），仅做金额类求和；3) 存在组合缺少总资产记录时，卡片下方提示"N 个组合暂无资产数据，未计入合计"                                                                                      |
| ACC-P0-04 | P0     | 组合列表卡             | 表格：组合名称 / 成立日 / 币种 / 最新总资产 / 累计净值 / 当年收益率 / 最后更新日                                     | 1) 点击行切换 `currentPortfolioId` 并跳转概览页；2) 当前组合行高亮；3) 提供"新建组合"按钮（复用 `PortfolioDialog`）；4) 与 `DASH-P1-01` **共用同一汇总接口，不重复开发**；5) 成立日列口径 = **首笔存入日**（`FIN-D6`，用户已确认保持，非创建日）：无首笔存入时显示「未成立」+ tooltip「成立日=首笔存入日，该组合尚无存入记录」，并带「创建于 yyyy-MM-dd」副行                                                                                                 |
...
| 编号        | 优先级 | 功能点          | 描述                                                                                         | 验收标准                                                                                                                                                                                                 |
| --------- | --- | ------------ | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SET-P0-01 | P0  | 账户资料与安全设置（**全站唯一修改入口**）        | 设置页账户区承载**全站唯一**的账户修改能力：**修改邮箱 / 修改密码 / 编辑资料（昵称·手机·简介）/ 头像修改（在「编辑资料」卡片内完成：本地上传 与 头像 URL 设置并列）**；区块顶部保留「头像 + 昵称 + 邮箱」一行摘要 + 「前往账户中心 →」链接                             | 1) 四类修改动作**全部且仅**在本区，复用现有 `ChangeEmailDialog` / `ChangePasswordDialog` / `EditProfileDialog`，其中 `ChangeEmailDialog` / `ChangePasswordDialog` **不要求改动内部实现**，`EditProfileDialog` 须按验收 2 内置「头像 URL」输入框；2) 头像修改支持**本地上传**（已有）与**头像 URL 设置**（输入图片 URL 即可用作头像），**两种方式并列置于「编辑资料」卡片（`EditProfileDialog`）内**（草图见 §7.9），**不设独立的「应用」按钮**：URL 输入框内容随卡片「保存」一起提交生效，最后生效者覆盖前者；若头像由**本地上传**设置（站内相对路径，如 `/api/uploads/...`），URL 输入框**不回显图片路径**，以占位提示「已通过本地上传设置头像」，仅外部 `http(s)` 图片地址允许粘贴；设置页账户区只展示头像并提供「编辑资料」入口，**不得再出现独立的「头像 URL」输入框**；3) **账户页 `/account` 不再提供任何修改入口**，本区为全站唯一入口，不得存在第二处账户资料修改入口；4) 手机号与简介的详细只读展示迁至账户页，本区只留摘要，与 `/account` 无信息重复；5) 修改成功后 toast 提示并刷新 profile 缓存                                                                                                                                    |
| SET-P0-02 | P0  | 偏好设置服务端化（🆕 含**默认日期范围全局化**） | 新增 `UserPreference` 模型（`userId` 唯一），字段见 §6.9.1。原 `localStorage` 方案迁移为服务端存储。🆕 **v3.3.0（原增量 I-04）**：设置页「默认日期范围」的**选项集合与全站「快捷范围」完全一致（7 项）**，且该偏好在全站所有范围型日期位置**全局生效** | 1) 保存后**换浏览器/换设备登录仍生效**；2) 首次登录返回系统默认值；3) 保存为乐观更新 + 失败回滚；4) 概览页 / 分析页启动时读取偏好作为默认值；5) 迁移逻辑：首次加载时若检测到旧 `localStorage` 值且服务端为空，自动上传一次；6) 🆕 **设置页「默认日期范围」下拉 = 7 项**（`1w` 近一周 / `1m` 近 1 月 / `3m` 近 3 月 / `6m` 近 6 月 / `1y` 近 1 年 / `ytd` 今年 / `all` 全部），与快捷范围**逐项 value+label 一致**（比对测试把关）；7) 🔴 🆕 **单一真相源 = `QUICK_RANGE_OPTIONS`**（`features/query/dimension-switcher.tsx`），设置页**复用该常量**；**全站不得存在第二份快捷范围选项数组**（代码评审：设置页不得再定义独立 `DATE_RANGE_OPTIONS`）；8) 🆕 **全局生效 8 处**：① 概览页「趋势分析」筛选栏（`DASH-P0-02`）② 收益分析页 `/analysis/xirr` ③ 净值分析页 `/analysis/nav` ④ 出入金列表页 `/cashflows` ⑤ 资产记录页 `/snapshots` ⑥ 现金余额变更历史 ⑦ 持仓页统一筛选器（`HOLD-B-P0-11`）⑧ 后续新增的任何**范围型**日期位置（`SYS-P1-03`）；9) 🆕 **优先级：URL 参数 > 用户偏好 > 系统默认 `1y`**；偏好为空 / 非法值 / 首次登录 → 回落 `1y`；10) 🆕 **后端取值域同步扩为 7 项**（`defaultDateRange` 保持 String 字段 + 服务端 `@IsIn` 白名单校验，**零 migration**，架构裁决 Q-5），`1w` / `6m` 等新值可持久化、换设备后仍生效；11) 🆕 不破坏既有默认「按月 + 近 1 年」（`DASH-P0-02` 验收 2 回归） |
| SET-P0-03 | P0  | 数据导出（⚠️ **不含费用记录**，见 `SET-P2-05`）         | 导出当前组合 **7 类数据**：**标的主数据（securities）/ 证券买卖流水 / 出入金 / 现金余额 / 证券价格 / 总资产记录 / 净值序列（navSeries = 每日净值 + 每日 XIRR 合并列）**，格式 **`csv` 或 `xlsx`（Excel 扩展，决策 Q-6 乙 + 用户新增）**（复选，多选后前端**串行逐个下载**，不打包 zip） | 1) `GET /api/portfolios/:id/export?type=&format=csv\|xlsx`，`format` 缺省 `csv`；CSV 为 UTF-8 **带 BOM**，Excel 打开中文不乱码；2) 文件名格式 `{组合名}_{类型}_{YYYYMMDD}.{csv\|xlsx}`；3) 5 年数据（约 2000 行/表）导出 < 3s；4) 总资产记录导出必须含 `source` 与 `valuationFlag` 列；5) 英文表头 + 第二行 `#` 注释行；Decimal 一律以**字符串原样输出**（不科学计数、不丢精度）；6) 导出行数与页面数据完全一致；7) 文件下载接口**不套响应信封**（`@Res()` 直出），前端 `responseType:'blob'`；8) ⚠️ **v3.3.0 澄清**：`ExportType` 当前**无 `FEES`（费用记录）类别**，故费用数据（含 `HOLD-B-P0-10` 新增的 `scenario` 场景列）**不在本条导出范围内**；补齐已登记为 **`SET-P2-05`**（P2） |
| SET-P0-04 | P0  | 数据导入         | CSV/Excel（`.csv` / `.xlsx` / `.xls`）导入**出入金 / 证券买卖流水 / 总资产记录**，提供模板下载，**preview / commit 两阶段** | 1) 提供三套模板（csv/xlsx 双格式下载）；2) **预览阶段**（`POST .../import/preview`）展示前 10 行并显示识别到的总行数，**绝不落库**；3) 逐行校验，错误报告含**行号 + 字段 + 原因**（错误可导出）；4) 提供「跳过错误行」/「全部回滚」两种模式，**默认全部回滚**（`Q-10`）；5) **提交阶段**（`POST .../import/commit`）在**单个 Prisma 事务**内写入，事务提交后**全流程仅调用一次** `recalculateNavRange(pid, minDate)`（自最小导入日期至今日，不逐笔触发）；6) 500 行导入 + 重算 < 10s；7) 总资产记录导入走 upsert（按 `(portfolioId, date)`，遵守每日唯一约束），**`source` 强制 `MANUAL`**；`securityTrades`/`cashFlows` **纯 insert 不去重**（同日多笔合法），预览对疑似重复行给 **warning（不阻断）**；8) 上传限制 `.csv/.xlsx/.xls`（MIME + 后缀双校验）、≤ 5MB、行数 ≤ 10000；超限返回明确错误码而非 500；9) 导入前 UI 提示用户先导出备份 |
...
| SET-P1-06 | P1  | **注销账户**（**v3.1.1 由账户页迁入**，原 `ACC-P1-03`）  | 设置页**危险操作区**（与 `SET-P0-05`「清空当前组合数据」同区，置于其下方并视觉上更高危）：**软删除账户及全部数据**。**全站唯一注销入口**；账户页不再提供 | 1) 需**输入邮箱二次确认**才能提交；2) **软删除**：置 `deletedAt = now`，账户及其全部组合数据**原样保留 30 天（冷静期）**，期间账户不可登录、数据不参与任何业务查询，**不做任何物理删除**；🔴 **注销后该账户已签发的 JWT 令牌立即失效**（`JwtAuthGuard` 检查 `deletedAt`，软删用户的旧 token 即时失效，无需等待 token 自然过期）；3) **冷静期内可自助恢复**：用原邮箱 + 正确密码在**登录页**尝试登录时，后端返回**冷静期业务信号**（而非"邮箱或密码错误"），登录页据此展示剩余天数与「恢复账户」入口 —— 恢复机制与接口契约见 **`SYS-P1-02` / §6.10.1**；4) **到期失效**：`deletedAt` 满 30 天后由 CleanupService **物理硬删**，此后恢复不再可能，登录页恢复为通用"邮箱或密码错误"提示；5) 需**明确的法律 / 隐私文案**，须写明三件事：数据留存 30 天、**30 天内可在登录页用原邮箱 + 密码自助恢复**、到期后彻底删除且不可找回；**严禁**出现"如需恢复请联系客服"等**人工恢复**表述（本应用无管理后台、无客服代恢复通道）；6) 与 `SET-P0-05`（仅清空单个组合数据、保留账户）**语义严格区分**，弹窗须明示"注销将删除账户本身及全部组合"；7) 提交成功后清空 `auth.store` 并跳转 `/login`；8) 账户页 `/account` 及整个账户模块**不得再出现注销入口**，不得重复实现 |
...
| **SET-P2-05** | **P2** | 🆕 **费用记录导出（含买入 / 卖出场景列）** | **背景**：`HOLD-B-P0-10`（v3.3.0）为 `FeeRecord` 新增了 `scenario`（买入时 / 卖出时）并落地了合并展示，原增量 PRD 验收项「导出费用数据新增 `scenario` 列」在实施时被判定**前提不成立** —— `packages/backend/src/modules/export/export-schemas.ts` 的 `ExportType` **仅有 7 类**（`securities` / `securityTrades` / `cashFlows` / `cashBalances` / `securityPrices` / `assetSnapshots` / `navSeries`），**从来就没有 `FEES` 费用类别**，因此"给费用导出加一列"无处可加（QA 报告遗留 #3，用户已裁决：**放入 P2**）。**范围**：① 在 `ExportType` 新增 **`FEES`** 导出类别（导出类别由 7 → 8）；② 定义费用导出列：`date`（日期）/ `securityCode`（标的代码）/ `securityName`（标的名称）/ `type`（费用类型 COMMISSION·STAMP_TAX·OTHER）/ **`scenario`（场景 BUY·SELL）** / `amount`（金额）/ `transactionId`（关联流水 ID，可空）/ `note`（备注）；③ 设置页数据管理区导出复选框新增「费用记录」；④ 导出**以明细行为准**（与 `HOLD-B-P0-10` 的展示层聚合口径一致：合并只在 UI 展示，导出不聚合），若后续需要合并口径需另行评审；⑤ 同步扩展 `GET /api/portfolios/:id/export?type=fees`。**是否一并补「分红记录（含 tax / netAmount）」导出留待评审时确认（当前不在本条范围内）** | 1) `type=fees` 可导出 `csv` / `xlsx` 两种格式，遵循 `SET-P0-03` 全部通用约定（UTF-8 带 BOM、英文表头 + `#` 注释行、Decimal 字符串原样输出、文件名 `{组合名}_fees_{YYYYMMDD}.{csv\|xlsx}`、不套响应信封）；2) **`scenario` 列必须存在**且取值为 `BUY` / `SELL`（注释行标注中文「买入时 / 卖出时」）；3) 导出行数 = 持仓页【E】费用板块**明细行**行数（非合并行数），且与当前筛选结果一致；4) 设置页导出复选框新增「费用记录」，多选时沿用串行逐个下载（不打包 zip）；5) 5 年数据（约 2000 行）导出 < 3s；6) 空数据时导出仅含表头的合法文件，不报错。**依赖**：`HOLD-B-P0-10`（`FeeRecord.scenario` 字段，✅ 已实现）+ `SET-P0-03`（导出框架，✅ 已实现）；**无新增数据模型、无 migration** |
...
| `defaultDateRange` | String，取值域 **7 项**：`1w/1m/3m/6m/1y/ytd/all`（v3.3.0 由 `3m/1y/ytd/all` **4 项扩展**） | `1y` | 默认日期范围快捷项。🔴 **取值域 = `QUICK_RANGE_OPTIONS` 单一真相源**，与全站快捷范围逐项一致（`SET-P0-02`）。**保持 Prisma String 字段 + 后端 `@IsIn` 白名单校验（零 migration，架构裁决 Q-5）**，不改为 Prisma enum。全局生效 8 处，优先级 **URL > 偏好 > `1y`** |
...
| `showLiquidated` | Boolean | `false` | 持仓列表「显示已清仓」开关初值（`HOLD-B-P0-11`，URL `closed` 参数优先级更高） |
...
| 区块 | 承载需求 | 说明 |
|------|---------|------|
| **账户与会话区** | `SET-P0-01`（账户资料与安全设置 · 全站唯一修改入口）、**`SET-P0-09`（退出登录）** | 区块顶部为「头像 + 昵称 + 邮箱」摘要 + 「前往账户中心 →」；四类修改动作在此；**退出登录**置于本区底部。**头像 URL 设置位于「编辑资料」卡片（`EditProfileDialog`，草图见 §7.9）内，本区不直接提供该输入框**，仅展示头像 + 提供「编辑资料」入口 |
...
| SYS-P1-02 | P1 | **账户注销冷静期与自助恢复**（🆕 新增，配套 `SET-P1-06`） | 为 `SET-P1-06` 的软删除提供**全站唯一**的恢复通路：**登录页冷静期信号** + `POST /api/auth/account/restore` **自助恢复接口**。**不设管理后台、不设管理员代恢复**（本应用为"每账户独立"模型，无运营角色） | 1) 对**软删且未满 30 天**的账户，以**原邮箱 + 正确密码**调用登录接口时，**不得**返回通用"邮箱或密码错误"，须返回**专属业务码**（冷静期信号）+ **剩余可恢复天数**；2) **枚举防护**：密码错误、账户不存在、账户已硬删三种情况一律返回**同一条**通用"邮箱或密码错误"，**不得**泄露"该邮箱存在软删账户"；3) `POST /api/auth/account/restore`（入参 `email` + `password`）校验密码通过且 `deletedAt` 在 30 天内 → **清空 `deletedAt`** → **签发新 JWT** → 用户直接进入登录态，无需二次登录；4) 恢复后账户资料、偏好与**全部组合数据与注销前完全一致**（冷静期内无物理删除，故无数据损失）；5) 恢复为**幂等安全**操作：对未注销的正常账户调用 restore 应失败（见 §6.10.1 错误码），不得被当作登录后门；6) 全部失败分支的错误码与文案见 **§6.10.1**；7) **须与 `SET-P1-06` 同批交付** —— 不得先上线注销、后补恢复，否则 PRD 对用户承诺的"30 天可恢复"不成立 |
| **SYS-P1-03** | **P1** | 🆕 **日期选择器统一规范（范围型必备快捷范围）**（v3.3.0 新增，原增量 I-06） | 全站日期选择位置分为**范围型**（起止筛选）与**单点型**（表单录入单日期 / as-of）两类：**范围型一律提供快捷范围**（复用 `DateRangeQuickPicker` 或 `DimensionSwitcher` 的 `quickRanges`，7 项来自单一真相源 `QUICK_RANGE_OPTIONS`）；**单点型保持单日期选择、不做快捷选择**（已裁决，见 §12.1 `I-Q-7`）。本条为**跨模块规范类需求**，无独立数据模型 | 1) 🔴 **全站范围型日期位置 100% 提供快捷范围**，行为与既有组件完全一致 —— 已覆盖 7 处：概览「趋势分析」/ 收益分析 `/analysis/xirr` / 净值分析 `/analysis/nav`（`DimensionSwitcher`）+ 出入金 `/cashflows` / 资产记录 `/snapshots` / 现金余额变更历史 / **持仓页统一筛选器**（`DateRangeQuickPicker`）；2) **统一组件契约**：快捷项 7 项；选择快捷项按 `resolveQuickRange(v, { allRangeStart })` 覆盖起止并回调 `quick`（供父级写 URL）；手动改起止 → `quick: undefined`、下拉回落占位；「全部」起始日 = 组合首个交易日（`Portfolio.baseDate`），缺省回落 `2000-01-01`；受控模式由父级驱动回显，值不在选项集内（如 `custom`）渲染占位「选择范围」；默认值接 `SET-P0-02`（URL > 偏好 > `1y`）；3) 🔴 **禁止各自实现**：范围型筛选场景**不得出现成对裸 `<input type="date">` 自实现的日期范围控件**（代码评审防回归，新增即失败）；`type="date"` 仅允许出现在 ① 共享组件内部（`date-range-quick-picker` / `dimension-switcher` 的起止对）② 单点型表单 ③ as-of 单点；4) **单点型位置行为不变（不回归）**，共 6 处记录在案：出入金录入/编辑日期、证券买卖录入/编辑日期、分红/费用录入/编辑日期、现金余额生效日期（asOf）、总资产记录手工新建/编辑日期、持仓日期（as-of，已并入 `HOLD-B-P0-11` 统一筛选器）；5) 审查矩阵（13 位置）随本 PRD 维护，新增页面须登记归类，QA 按矩阵逐位置点检留痕 |
| SYS-P2-01 | ~~P2~~ | ~~多端数据同步~~ **已移除** | ~~Web 与 HarmonyOS APP 实时/准实时同步~~ —— `packages/harmonyos` 包已删除，仅保留 Web 端 | — |
| SYS-P2-02 | ~~P2~~ | ~~HarmonyOS APP 端~~ **已移除** | ~~ArkTS + ArkUI 原生鸿蒙应用~~ —— `packages/harmonyos` 包已删除 | — |
| SYS-P2-03 | P2 | 多币种 | 外币 + 汇率折算 | v1 单币种 CNY |

### 6.10.1 账户自助恢复契约（`SYS-P1-02` · 配套 `SET-P1-06`）

> **产品意图层定义**：本节只定义**产品级契约**（入参 / 出参 / 约束 / 错误分支）。完整技术契约（DTO、HTTP 状态码取值、业务码常量名）以 `docs/ARCHITECTURE.md` §4.2 为权威，两端须一致。

**① 状态机（唯一权威）**

| 账户状态 | `deletedAt` | 登录接口行为 | restore 接口行为 |
|---|---|---|---|
| 正常 | `null` | 正常签发 JWT | **拒绝**（`ACCOUNT_NOT_DELETED`） |
| 冷静期内 | 非空，距今 **< 30 天** | 返回**冷静期信号** + 剩余天数（**不返回**"邮箱或密码错误"） | **允许恢复** → 清空 `deletedAt` + 签发新 JWT |
| 已硬删 | 记录已物理删除 | 返回通用"邮箱或密码错误" | 失败（等同账户不存在） |

> **边界口径**：剩余天数 = `ceil(30 - (now - deletedAt) / 1 天)`，取值 `1~30`；`now - deletedAt >= 30 天` 即视为过期，即便 CleanupService 尚未跑批也**不得**恢复。

**② `POST /api/auth/account/restore`**

| 项 | 定义 |
|---|---|
| **鉴权** | **免 JWT**（用户此时无有效会话），须与登录接口同级做**限流 / 防爆破**（复用登录接口的失败次数限制） |
| **入参** | `email`（必填，字符串）、`password`（必填，字符串，明文经 HTTPS 传输，后端 bcrypt 比对） |
| **出参（成功）** | `token`（新签发 JWT）+ `user`（与登录接口一致的用户对象）→ 前端直接写入 `auth.store` 并跳转概览页 |
| **前置约束** | ① 账户存在；② `deletedAt` 非空；③ `now - deletedAt < 30 天`；④ 密码校验通过。**四条全部满足**方可恢复 |
| **副作用** | 清空 `deletedAt`（置 `null`）；**不**改动任何组合 / 流水 / 快照数据；**不**重置密码或偏好 |

**③ 错误分支（全部须覆盖）**

| 场景 | 结果 | 对外文案 | 备注 |
|---|---|---|---|
| 邮箱不存在 | 失败 | "邮箱或密码错误" | 与"密码错误"**同一文案**，防账户枚举 |
| 密码错误 | 失败 | "邮箱或密码错误" | **即便账户确在冷静期内**也不得提示"该账户可恢复" |
| 账户未注销（`deletedAt = null`） | 失败 | "该账户无需恢复，请直接登录" | 防止 restore 被当作绕过登录的后门 |
| 软删已超 30 天（记录仍在，跑批未执行） | 失败 | "恢复期已过，账户数据已不可找回" | **按期判定，不按跑批判定** |
| 已硬删（记录不存在） | 失败 | "邮箱或密码错误" | 等同邮箱不存在 |
| 限流触发 | 失败 | "尝试过于频繁，请稍后再试" | 与登录接口共用计数 |

**④ 登录接口的冷静期信号**

| 项 | 定义 |
|---|---|
| **触发条件** | 邮箱匹配到软删账户 **且** 密码校验**通过** **且** 未满 30 天（三条同时满足） |
| **返回内容** | 专属**业务码**（表示"账户处于注销冷静期"，取值由架构定义）+ **剩余天数** |
| **不触发** | 密码错误 / 账户不存在 / 已超 30 天 → 一律走通用"邮箱或密码错误"分支 |
| **前端契约** | 登录页识别该业务码后**不显示错误提示**，改为展示恢复引导（草图见 **§7.10**） |

### 6.10.2 日期选择器审查矩阵（`SYS-P1-03` · v3.3.0 落地基线）

> 全站日期选择位置的**权威清单**。新增页面 / 组件若含日期选择，**必须在此登记并归类**；范围型缺快捷范围即视为不达标（`SYS-P1-03` 验收 1 / 3）。

| # | 位置（页面 / 组件） | 类型 | 落地状态 | 承载需求 |
|---|--------------------|------|---------|---------|
| 1 | 概览页「趋势分析」筛选栏（`DimensionSwitcher`） | 范围型 | ✅ 7 项快捷范围 + 接偏好默认 | `DASH-P0-02` / `SET-P0-02` |
| 2 | 收益分析页 `/analysis/xirr`（`DimensionSwitcher`） | 范围型 | ✅ 同上 | `ANL-P0-04` / `SET-P0-02` |
| 3 | 净值分析页 `/analysis/nav`（`DimensionSwitcher`） | 范围型 | ✅ 同上 | `ANL-P0-05` / `SET-P0-02` |
| 4 | 出入金列表页 `/cashflows`（`DateRangeQuickPicker`） | 范围型 | ✅ 同上 | `FLOW-P0-02` / `SET-P0-02` |
| 5 | 资产记录页 `/snapshots`（`DateRangeQuickPicker`） | 范围型 | ✅ 同上 | `SNAP-P0-04b` / `SET-P0-02` |
| 6 | 现金余额变更历史（`DateRangeQuickPicker`） | 范围型 | ✅ 同上 | `CASH-P1-01` / `SET-P0-02` |
| 7 | **持仓页统一筛选器**（`HoldingsToolbar`） | 范围型 | ✅ v3.3.0 新接入，7 项快捷范围 + 接偏好默认 | `HOLD-B-P0-11` / `SET-P0-02` |
| 8 | 出入金录入 / 编辑表单日期 | 单点型 | ➖ 不适用快捷范围，保持单日期 | `FLOW-P0-02` |
| 9 | 证券买卖录入 / 编辑表单日期 | 单点型 | ➖ 同上 | `HOLD-B-P0-02` |
| 10 | 分红 / 费用录入 / 编辑表单日期 | 单点型 | ➖ 同上 | `HOLD-B-P0-10` |
| 11 | 现金余额录入（生效日期 `asOf`） | 单点型 | ➖ 同上 | `CASH-P0-01` / `CASH-P0-02` |
| 12 | 总资产记录手工新建 / 编辑日期 | 单点型 | ➖ 同上 | `SNAP-P0-06` |
| 13 | **持仓日期（as-of）** | 单点型（已并入统一筛选器） | ✅ v3.3.0 内化为统一筛选器的 as-of 字段，能力零丢失 | `HOLD-B-P0-11` |

> 🔴 **口径提醒**：第 13 项 as-of 虽然位于统一筛选器卡片内，但仍是**单点型**，与同卡片内的**范围型**日期范围**互不换算、各自驱动不同板块**（架构裁决 Q-6，见 `HOLD-B-P0-11` 验收 6）。

---

# §7 UI 草图（文字版 Wireframe）

> 📌 **修订注（2026-08-04）**：§7.7 / §7.8 / §7.9 已按当前前端实现同步目标态 —— 账户页四卡横排（`grid grid-cols-1 xl:grid-cols-12`：个人信息 3 / 资产全景 5 / 数据统计 4 / 我的组合 12）、设置页偏好「外观主题 / 软提示开关 / 金额格式 / 快照过期阈值」四块横排、编辑资料头像 URL 随「保存」提交（无独立「应用」按钮）。

## 7.1 出入金管理页 `/cashflows`

> 🔀 **v3.2.0 概览融合改版**：本页**不再展示总资产概览** —— 原【A】块（当前总资产 / 持仓市值 / 现金余额 3 卡 + 近 30 日走势图 + 手工记录标记 + 「查看全部历史 / ⚙ 管理历史记录」双入口）已整体迁移至概览页（见 §6.6 / §7.4），本页仅保留【B】现金余额（手工维护）与【C】出入金流水列表。
...
> 🔧 **交互边界**：
> - 本页仅保留【B】现金余额（手工维护）与【C】出入金流水两块 —— 原【A】总资产概览块（3 卡 + 近 30 日走势图 + 手工记录标记）已随 **v3.2.0 概览融合改版整体迁移至概览页**（见 §6.6 / §7.4），本页**不出现**任何总资产展示与走势图。
> - 历史总资产记录的浏览与管理**统一于 `/snapshots`**（本页不再提供「查看全部历史 / ⚙ 管理历史记录」双入口）。
> - 用户有两条修改总资产的路径 —— **改上游（推荐，默认）** 或 **直接手工补录/修正某日记录（例外，需二次确认，且会取代该日自动记录，在 `/snapshots` 完成）**。
...
│ 持仓        [组合: 我的组合▼]                  [+ 录入买卖]   │
├──────────────────────────────────────────────────────────────┤
│ 【F】🔍 统一筛选器（三板块共享）  v3.3.0       (HOLD-B-P0-11)│
│  ┌──────────────────────────────────────────────────────────┐│
│  │ 日期范围 [近 1 年 ▼] [2025-08-07]~[2026-08-07]  ← 范围型 ││
│  │          （7 项快捷范围，默认取偏好 defaultDateRange）    ││
│  │ 持仓日期(as-of) [2026-08-07 📅]  ← 单点型，默认今日       ││
│  │ 证券 [已选 2 项 ▼]        场景 [全部 ▼](买入/卖出/全部)   ││
│  │ ── ▸ 持仓专属筛选（可折叠）───────────────────────────── ││
│  │    类型[✓全部][股票][基金][债券][其他]  [ ] 显示已清仓    ││
│  │ ⓘ 持仓板块以「持仓日期」为准，买卖明细 / 分红费用以       ││
│  │   「日期范围」为准（两套口径独立，互不换算）              ││
│  └──────────────────────────────────────────────────────────┘│
│  URL: date/closed/types/sec/range/from/to/scenario（等于默认  │
│       值不写入；URL > 用户偏好 > 系统默认 1y）                │
├──────────────────────────────────────────────────────────────┤
│ 【A】持仓汇总（随筛选动态变化）                 (HOLD-B-P0-06)│
│ 总市值 ¥185,400 │ 总成本 ¥160,000 │ 浮盈 +¥25,400 (+15.88%)  │
│ 标的数 6        │ ⓘ 本页市值将自动计入每日总资产记录          │
├──────────────────────────────────────────────────────────────┤
│ 【B】持仓列表（由买卖流水推导 · 受 as-of / 证券 / 类型 /      │
│      显示已清仓 驱动；不受日期范围影响）        (HOLD-B-P0-04)│
│ ┌────────┬──────┬────┬──────┬──────┬─────────┬────────┬─────┐│
...
│      不受 as-of 影响 · 无独立筛选栏）           (HOLD-B-P0-07)│
│ ┌──────────┬──────┬────────┬─────┬───────┬──────┬─────┬────┐│
...
│ └──────────┴──────┴──────┴─────┴───────┴──────┴─────┴────┘│
│  ✎ 编辑 → 打开与「录入」完全相同的表单        (HOLD-B-P0-02) │
├──────────────────────────────────────────────────────────────┤
│ 【D】🍩 标的占比 / 类型占比 双环形图            (HOLD-B-P1-03)│
├──────────────────────────────────────────────────────────────┤
│ 【E】分红 / 费用记录（不参与收益计算 · 受 日期范围 / 证券 /   │
│      场景 驱动）                                (HOLD-B-P0-10)│
│  [分红记录 ▾]  [费用记录 ▾]   按标的汇总累计分红 / 累计费用    │
│                                                               │
│  ── 分红记录 Tab ──（净额 = 金额 − 所得税，后端统一计算）──── │
│  ┌──────────┬────────┬──────┬────────┬────────┬──────┬────┐  │
│  │日期      │标的    │类型  │金额    │所得税  │净额  │操作│  │
│  ├──────────┼────────┼──────┼────────┼────────┼──────┼────┤  │
│  │2026-07-15│贵州茅台│现金  │ 320.00 │  60.00 │260.00│✎ 🗑│  │
│  │2026-06-30│沪深300 │现金  │ 320.00 │      - │320.00│✎ 🗑│  │
│  └──────────┴────────┴──────┴────────┴────────┴──────┴────┘  │
│   所得税留空 = 0；tax > 金额 → 报错「净额不能为负」           │
│                                                               │
│  ── 费用记录 Tab ──（合并 = 展示层聚合，明细行原样保留）───── │
│  ┌──────────┬────────┬────────┬──────┬────────┬──────┬────┐  │
│  │日期      │标的    │场景    │类型  │合计金额│笔数  │操作│  │
│  ├──────────┼────────┼────────┼──────┼────────┼──────┼────┤  │
│  │2026-08-01│贵州茅台│[买入时]│佣金  │   8.50 │2 笔  │✎   │  │
│  │2026-08-01│贵州茅台│[买入时]│印花税│   1.72 │1 笔  │✎ 🗑│  │
│  │2026-07-20│沪深300 │[卖出时]│佣金  │   2.00 │1 笔  │✎ 🗑│  │
│  └──────────┴────────┴────────┴──────┴────────┴──────┴────┘  │
│   合并键 = (组合, 标的, 日期, 场景, 类型)；GET /fees?grouped=1│
│   合并行（笔数 > 1）删除禁用，✎ 作用于代表明细行              │
│   [买入时]/[卖出时] 为徽标；可被统一筛选器「场景」过滤        │
└──────────────────────────────────────────────────────────────┘

【录入 / 编辑证券买卖弹窗】—— 🔴 v3.3.0：录入与编辑为同一表单
┌────────────────────────────────────┐
│ 录入证券买卖 / 编辑证券买卖     [×]│
│ ⓘ 组合内部买卖，不计入出入金现金流  │
│ 方向* (●)买入  ( )卖出             │
...
│ 成交额*[17,205.00]                 │
│ ── 费用（编辑态同样展示并回填）──  │
│ 佣金  [5.00 ] 印花税[0.00] 其他[0] │
│ ────────────────────────────────   │
│ 含费单价（自动只读） 1,720.000000  │
│   = (成交额 − 费用合计) / 数量     │
│ 备注  [加仓                    ]   │
│            [取消]  [保存]          │
└────────────────────────────────────┘
  ⓘ 编辑态字段集合 / 顺序 / 校验 / 预览与录入态**完全一致**
  ⓘ 存量旧口径（trade.fee≠0 且无关联 FeeRecord）→ amber 提示
     「旧口径费用将并入含费单价」                (HOLD-B-P0-02)
  保存后：重建该笔 transactionId 关联的 FeeRecord（删旧插新），
          scenario 自动 = 该笔方向（买入时 / 卖出时）
...
> 🔀 **v3.2.0 概览融合改版**：本页为总资产概览的**唯一承载页** —— 指标卡 6→8（**资产构成 / 收益表现**两组，`DASH-P0-01`），新增「趋势分析」分区（日期筛选栏 + 总资产走势图 hero + 四宫格，`DASH-P0-02` / `SNAP-P0-04b`）；总资产走势图卡头为**单一 `/snapshots` 入口**（「查看全部历史」）。
...
│收益分析 │  ▍资产构成（4 卡）                                   │
│净值分析 │ ┌──────────┬──────────┬──────────┬──────────┐      │
│账户     │ │当前总资产 │ 持仓市值 │ 现金余额 │  净投入   │      │
│设置     │ │¥285,400  │¥185,400  │¥100,000  │¥120,000  │      │
│        │ └──────────┴──────────┴──────────┴──────────┘      │
│        │  ▍收益表现（4 卡）                                   │
│        │ ┌──────────┬──────────┬──────────┬──────────┐      │
│        │ │累计收益率 │ 当年收益率│ 年化XIRR │ 累计净值  │      │
│        │ │ +45.60%  │  +8.20%  │  12.34%  │  1.4560  │      │
│        │ └──────────┴──────────┴──────────┴──────────┘      │
│        │                                       (DASH-P0-01)  │
│        │ ▍趋势分析（筛选栏仅作用于本区图表）                   │
...
**UI 草图（四卡横排 · 只读形态 · 风格对齐 §7.4 概览页）**

```
┌────────────────────────────────────────────────────────────────────────────────┐
│ [侧栏]  │ 顶栏: [组合选择▼]                        [🔔] [👤头像▼]              │
├─────────┼──────────────────────────────────────────────────────────────────────┤
│ 概览    │ 账户中心 · 纯只读聚合视图（修改入口见设置页）                         │
│ 持仓    │ 布局：grid grid-cols-1 gap-6 xl:grid-cols-12（窄屏回退纵向）          │
│ 出入金  │ ┌──────────────────┬──────────────────────────┬──────────────────────┐ │
│ 资产记录│ │ ① 👤 个人信息   │ ② 📊 资产全景           │ ③ 📈 数据统计       │ │
│ 收益分析│ │ xl:col-span-3    │ xl:col-span-5            │ xl:col-span-4        │ │
│ 净值分析│ │ 头像 / 昵称 /    │ 组合数 3                 │ 出入金笔数 128       │ │
│>账户    │ │ 邮箱 / 手机(脱敏)│ 合计总资产 ¥520,800      │ 证券买卖笔数 340     │ │
│ 设置    │ │ 注册时间 / 简介  │ 合计净投入 ¥420,000      │ 总资产记录天数 612   │ │
│         │ │ ⓘ 卡内无任何修改│ 合计浮动盈亏 +¥100,800   │ 账户使用天数 887     │ │
│         │ │   控件，头像仅展示│ ⓘ 1 个组合暂无总资产记录│ 起始日期 2023-03-01  │ │
│         │ │ [前往设置修改 →] │ ⓘ 仅做金额类求和；      │ 最近日期 2026-08-03  │ │
│         │ │ (ACC-P0-05 ·     │   不做合计 XIRR(Q-07)    │ (ACC-P0-06)          │ │
│         │ │  SET-P0-01)      │ (ACC-P0-03)              │                      │ │
│         │ │  ← 全页唯一入口  │                          │                      │ │
│         │ └──────────────────┴──────────────────────────┴──────────────────────┘ │
│         │ ┌──────────────────────────────────────────────────────────────────┐ │
│         │ │  💼 我的组合（xl:col-span-12 独占整行 · 点击行=切换并跳转概览）     │ │
│         │ │  [+新建组合] (ACC-P0-04)                                            │ │
│         │ │ ┌───────────┬───────────┬────┬──────────┬───────┬───────┬──────┐ │ │
...
│         │ │ │ 新组合    │未成立     │CNY │  0       │ —     │ —     │ —    │ │ │
│         │ │ └───────────┴───────────┴────┴──────────┴───────┴───────┴──────┘ │ │
│         │ │ ⓘ 成立日列口径 = 首笔存入日（FIN-D6，用户已确认保持，非创建日）：│ │
│         │ │   无首笔存入 → 显示「未成立」+ tooltip「成立日=首笔存入日，该组合 │ │
│         │ │   尚无存入记录」+ 副行「创建于 yyyy-MM-dd」                       │ │
│         │ └──────────────────────────────────────────────────────────────────┘ │
│         │ ⓘ 本页零修改、零登出（v3.1.1）：                                     │
...
│         │ └──────────────────────────────────────────────────────────────────┘ │
│         │ ┌──────────────────────────────────────────────────────────────────┐ │
│         │ │ ② ⚙ 偏好与外观区     (SET-P0-02 · 07 · 08 / SET-P1-03 · 05)      │ │
│         │ │ 默认组合 [A股组合 ▼]（可选「不设置」；登录后自动选中）           │ │
│         │ │ ── 以下两两并排（grid sm:grid-cols-2）────────────────────────── │ │
│         │ │ 默认时间维度 [按月 ▼]    默认日期范围 [近 1 年 ▼]                │ │
│         │ │   ⓘ 默认日期范围 = 全站「快捷范围」同一组 7 项（v3.3.0）：       │ │
│         │ │     近一周 / 近1月 / 近3月 / 近6月 / 近1年 / 今年 / 全部          │ │
│         │ │     单一真相源 QUICK_RANGE_OPTIONS，全局生效 8 处；              │ │
│         │ │     优先级 URL > 偏好 > 1y                      (SET-P0-02)      │ │
│         │ │ 周期聚合方式 [期末值 ▼]  周起始日 (●)周一 ( )周日                │ │
│         │ │ 净值小数位 [4▼]          XIRR 小数位 [2▼]                       │ │
│         │ │ ── 四块横排（grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4）─── │ │
│         │ │ ┌────────────────┬────────────────┬────────────────┬────────────┐ │ │
│         │ │ │ 外观主题       │ 软提示开关     │ 金额格式       │ 快照过期阈值│ │ │
│         │ │ │ (●)跟随系统    │ [x]出入金后提示│ [x]千分位      │ [ 3 ] 天   │ │ │
│         │ │ │ ( )亮色( )暗色 │ [x]买卖后提示  │ [ ]万/亿缩写   │ 范围 1~30  │ │ │
│         │ │ │ (SET-P0-08)    │ (SET-P0-07)    │ (SET-P1-03)    │ (SET-P1-05)│ │ │
│         │ │ └────────────────┴────────────────┴────────────────┴────────────┘ │ │
│         │ │ 货币 [人民币 CNY ▼]  语言 [中文（简体）▼] —— 待后端集成，暂禁用    │ │
│         │ │                                           [保存偏好] (SET-P0-02) │ │
│         │ └──────────────────────────────────────────────────────────────────┘ │
│         │ ┌──────────────────────────────────────────────────────────────────┐ │
...
│         │ │ ⓘ 「设为默认」已落地（SET-P0-06）：★ 星标按钮写偏好              │ │
│         │ │   defaultPortfolioId 并切换当前组合；默认组合星标高亮；          │ │
│         │ │   已归档组合不可设为默认（按钮禁用）                             │ │
│         │ └──────────────────────────────────────────────────────────────────┘ │
│         │ ┌──────────────────────────────────────────────────────────────────┐ │
...
│         │ │ ──────────────────────────────────────────────────────────────── │ │
│         │ │ 注销账户（软删除账户本身及全部组合，冷静期 30 天）               │ │
│         │ │   二次确认：输入邮箱 + 法律/隐私文案      [注销账户] (SET-P1-06) │ │
│         │ │   文案须写明：30 天内可在登录页用原邮箱 + 密码自助恢复；         │ │
│         │ │   超 30 天由 CleanupService 硬删，彻底不可找回       (SYS-P1-02) │ │
│         │ │   ⚠ 严禁出现"如需恢复请联系客服"（本应用无客服代恢复通道）       │ │
│         │ └──────────────────────────────────────────────────────────────────┘ │
└─────────┴──────────────────────────────────────────────────────────────────────┘
```
...
│        ╰────╯  限制：jpg / png / webp，≤ 2MB                   │
│                                                                │
│ 头像URL [https://example.com/avatar.png        ]                          │
│        ⓘ 本地上传 与 URL 输入两种方式并列，均随「保存」一起提交生效；     │
│          最后生效者覆盖前者。无独立「应用」按钮（与「保存」合并）          │
│          （SET-P0-01 验收 2）                                              │
│        ⓘ 若头像由本地上传设置（站内路径），URL 输入框不回显图片路径，     │
│          以占位提示「已通过本地上传设置头像」                              │
│ ────────────────────────────────────────────────────────────── │
│ 昵称   [张三                          ]   ≤ 20 字              │
...
> **本节只定义产品意图与文案口径**；登录页整体布局属既有实现，本 PRD 不重绘，仅新增下方**冷静期分支**。业务码取值、组件拆分、动效等前端细节由架构 / 工程落地。

**状态 A · 常规失败（邮箱不存在 / 密码错 / 已硬删）—— 行为不变**
...
**状态 B · 命中冷静期信号（邮箱 + 密码均正确，软删未满 30 天）**
...
| 项 | 口径 |
|---|---|
| **触发** | 登录接口返回**冷静期业务码**时（三条件见 §6.10.1 ④），**不显示**"邮箱或密码错误"，改渲染状态 B 卡片 |
| **剩余天数** | 直接取登录接口返回值，前端**不自行计算**；显示为整数天，最小 `1 天` |
| **「恢复账户」** | 复用页面已填的 `email` + `password` 调 `POST /api/auth/account/restore`；成功 → 写入新 JWT → 跳转概览页（**不要求用户再登录一次**） |
| **「暂不恢复」** | 关闭提示、清空密码框，回到状态 A 的空白登录态；**不**登出、不产生任何服务端副作用 |
| **恢复失败** | 按 §6.10.1 ③ 错误分支展示对应文案（如"恢复期已过，账户数据已不可找回"） |
| **禁止项** | 全站登录 / 注销链路**不得**出现"如需恢复请联系客服"等人工恢复表述 |
...
- PostgreSQL 为唯一数据源（SSOT）；写入经后端 API。~~APP 端离线录入（P2）冲突以服务器为准~~ —— HarmonyOS APP 端已移除（`packages/harmonyos` 包已删除），仅保留 Web 端。
- ~~Web 与 HarmonyOS 共用后端 API（PRD.md §7）；本版 v2.3 范围不含 `harmonyos`（PRD-modules Q-14）。~~ **HarmonyOS 端已移除，仅 Web 端。**

### 9.4.1 ~~Web 端与 HarmonyOS APP 端交互差异~~（已移除）

> **本节已移除**：`packages/harmonyos` 包已删除，不再有 HarmonyOS APP 端。系统仅保留 Web 端，以下交互差异表仅作历史参考，不构成验收项。
>
> ~~**范围说明**：HarmonyOS 端为 P2 范围（SYS-P2-02，本阶段不交付）~~ —— 该端已移除。Web 端为唯一实现基准。
...
> **口径不变量**：**所有金融计算（XIRR / NAV / 持仓推导 / 总资产派生）一律在后端完成**，前端仅做格式化展示（FG3 / C-08）。HarmonyOS 端已移除，系统仅保留 Web 端，此不变量对 Web 端完全适用。
...
| `GET /api/portfolios/:id/snapshots/:date` | P0 | 🆕 **A3 单条端点**（8 页对齐落地）：查询指定日期单条快照，`date` 必须 `YYYY-MM-DD`（否则 400），该日无记录 → 404；响应含 `derivedTotalAsset`（同列表口径） |
...
| `GET /api/portfolios/comparison` | P0/P1 | PRD-modules §3.5 |
| `GET /api/portfolios/:id/metrics/drawdown` | P1 | PRD-modules §3.5 |
| `GET/PATCH /api/users/preferences` | P0 | PRD-modules §7.6 |
| `GET /api/portfolios/:id/export?type=&format=` | P1 | 🆕 **8 页对齐落地**：导出 7 类（`securities/securityTrades/cashFlows/cashBalances/securityPrices/assetSnapshots/navSeries`），`format=csv\|xlsx`（缺省 csv），文件直出不套信封（对齐 SET-P0-03 / FLOW-P1-01） |
| `GET /api/data-transfer/template?type=&format=` | P1 | 🆕 **8 页对齐落地**：下载导入模板（3 类，csv/xlsx 双格式），文件直出不套信封（对齐 SET-P0-04） |
| `POST /api/portfolios/:id/import/preview` | P1 | 🆕 **8 页对齐落地**：导入预览（multipart：file + type，**不落库**），返回前 10 行 + 全量行级错误 + token（对齐 SET-P0-04 / FLOW-P1-01） |
| `POST /api/portfolios/:id/import/commit` | P1 | 🆕 **8 页对齐落地**：导入提交（`{ type, token }`），单 Prisma 事务 + 全流程仅 1 次重算，返回 `{inserted, updated, skipped, failed, recalculated}`（对齐 SET-P0-04 / FLOW-P1-01） |
| `DELETE /api/portfolios/:id/data` | P0 | PRD-modules §7.6 |
| `GET /api/account/stats` | **P0** | PRD-modules §6.5 → `ACC-P0-06`（原 `ACC-P1-01` P1 已升级） |
| `POST /api/auth/account/restore` | P1 | 🆕 `SYS-P1-02` 自助恢复账户（免 JWT，入参 email + password，契约见 §6.10.1） |
| `POST /api/auth/login`（**冷静期信号扩展**） | P1 | 🆕 `SYS-P1-02` 软删账户 + 密码正确时返回冷静期业务码 + 剩余天数（见 §6.10.1 ④） |
