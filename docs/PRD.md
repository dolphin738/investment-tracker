...
| SNAP-P0-02      | P0     | 总资产卡数据契约                     | 见 `FLOW-P0-03`（同一区块，此处定义数据契约）                                                                                       | 返回 `{ totalAsset, marketValue, cashBalance, priceAsOf, cashAsOf, date, source, valuationFlag, recordedAt, derivedTotalAsset }`；`source` 为 `DERIVED` 或 `MANUAL`；当 `source='MANUAL'` 时 `derivedTotalAsset` 由 **`computeDerived(date)` 实时计算**得出（**不是**从库中另一行读取），当 `source='DERIVED'` 时该字段**等于 `totalAsset`**（该行本就是系统派生结果，不重复计算）；计算失败/数据缺失时该字段为 `null` 且接口**照常 200**（派生值是增强信息，不是核心数据）                                                                                                                                                         |
...
| DASH-P0-02 | P0 | 时间维度切换器 | 顶部提供 `日/周/月/年` Tab + 日期范围选择器（快捷项：**近一周 / 近 1 月 / 近 3 月 / 近 6 月 / 近 1 年 / 今年 / 全部（7 项，决策 Q-6 乙）**） | 1) 复用 `features/query/dimension-switcher.tsx`；2) **默认值「按月 + 近 1 年」**，可被用户偏好覆盖（`SET-P0-02`），偏好为空时回落到「月 + 近 1 年」；3) 切换后两张图表同步刷新；4) **维度与范围写入 URL query**（key：`g`/`range`/`from`/`to`，等于默认值不写入 URL，刷新/分享后保持）；5) 单次查询响应 < 500ms |
