...
| §8 | 计算引擎契约 |
| §9 | 非功能性需求 |
| §10 | 🔴 **验收与强制回归测试**（P0 强制门禁 · REG-01~REG-05） |
| §11 | 风险登记册 |
| §12 | 待确认问题（已裁决 / 仍开放） |
...
| **C-09** | 持仓写操作**一律不得触发** `triggerCalculation` / `recalculateFromDate` | 🔴 **废止** | 新约束 **C-09′**：证券买卖 / 现价更新 **必须**触发 `persistDerived` 区间重建 + `recalculateFromDate`（五类触发事件之一，见 `SNAP-P0-03`） | 持仓已成为总资产来源，不触发就会产生过期资产值（W-4） |
...
| **C-09′** | 🔴 **五类事件必须触发重算**                          | 出入金 / 证券买卖 / 现价更新 / 现金余额变更 / 手工总资产记录增改删重置 —— 任一写操作都必须走 `recalculation.service` 统一入口                                                                                                                                                                                                                         | **改写自 C-09**               |
...
> 🔴 **本章为 P0 强制门禁（Release Gate）**：`REG-01` ~ `REG-05` 五项**必须全部通过**，未全部通过**不得进入交付 / 上线**。**不接受"部分通过 + 上线后补测"，不接受任何形式的豁免或降级。**
>
> **来源**：`PRD-incremental-v2.3.md` §9.3「必须覆盖的强制回归测试（5 项）」，原文五条**逐条原样搬入**，本章为**唯一集中口径**。
>
...
| 通过标准 | `REG-01` ~ `REG-05` **全部通过**；任一失败 = 交付阻塞（Blocker） |
...
| REG-01 ~ REG-05（全部） | **W-5**（§11 风险登记册） | 本章是 W-5 的**唯一验收闸门** |
...
| 🔴 W-5 | 手工记录被自动重算误删/误覆盖（最高危） | 每日唯一模型下两条写路径竞争；漏写 `source` 条件即覆盖且不可恢复 | 统一 `persistDerived()` 内部强制条件 + `ON CONFLICT DO NOTHING`；禁止其他路径直写；🔴 **验收闸门 = §10 `REG-01`~`REG-05`（P0 强制门禁，未全通过不得交付）** |
...
| 原ID | 类型 | 状态 | 新 PRD 落位 | 标记 |
|------|------|------|------------|------|
| §9.3 强制回归测试 5 项（①~⑤） | 验收门禁 | 承袭（**v3.0 首版被打散进 SNAP-P0-04a / CASH-P0-03 / W-5；本次修订提升为独立顶级章节并给稳定 ID**） | **§10 `REG-01`~`REG-05`**（散落处保留并加交叉引用） | ✅ |
