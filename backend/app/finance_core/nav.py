"""净值计算（单位份额法，纯函数）。对齐 docs/ARCHITECTURE.md §7.2。

口径：
- 成立日（无 prev）：首笔必须买入，shares=买入金额，净值=1.0
- 非成立日：unit_nav = 当日资产 / 上日份额；cumulative_nav = unit_nav
...
def compute_daily_nav(
    ...
    """计算指定日期净值。返回 NavResult；成立日首笔非买入抛 ValueError。"""
    if prev is None:
        # ===== 成立日 =====
        if day_buy <= 0:
            raise ValueError("首笔交易必须为买入（day_buy > 0）")
        return NavResult(
    ...
    # ===== 非成立日 =====
    # 防御：上日份额为 0（曾全赎回）或当日资产为 0，单位净值沿用上日累计净值，避免除零
    if prev.shares == 0 or snapshot_total == 0:
        unit_nav = prev.cumulative_nav
    else:
        unit_nav = snapshot_total / prev.shares
