 ...
 * 从 @/lib/types import 并 re-export，仅 shared 中不存在的 UI 辅助类型
 ...
 *
 * 🔴 命名消歧（T01 共享类型归并）：`@/lib/types` 里另有一个同名的
