/**
 * modules/holdings/trade-security-filter.ts — 买卖明细标的过滤派生（缺陷4 二次修复）
 *
 * 回归重点：**空结果不得退化成「不过滤」**。
 * 旧实现把「无约束 / 字典未就绪 / 筛选无命中」三种语义都压成空数组，
 * 调用方据此省略 securityId，后端返回全量交易 —— 表现为「类型筛选器无效」。
 *
 * 移植自 React 版 web/src/features/holdings/__tests__/trade-security-filter.test.ts
 */

import { describe, expect, it } from 'vitest';
import { SecurityType } from '@/lib/types';
import type { Security } from '@/api/types';
import { deriveTradeSecurityFilter } from '@/modules/holdings/trade-security-filter';
