/**
 * modules/overview/__tests__/stat-card.test.ts — 概览指标卡组件测试
 *
 * 覆盖（B4 批次验收：统计卡渲染）：
 * 1. 基础渲染：标题 + 数值 + tabular-nums 等宽数字类；中性态无涨跌箭头
...
describe('StatCard — 概览指标卡', () => {
  it('基础渲染：标题、数值与 tabular-nums 类；无 trend 时不渲染箭头', () => {
    const wrapper = mount(StatCard, {
      props: { title: '当前总资产', value: '¥205,000.00' },
    });
  ...
  it('change 与 description 同时提供时以「·」分隔渲染', () => {
    ...
    const onlyChange = mount(StatCard, {
      props: { title: '累计净值', value: '1.1235', change: '+2.1pp' },
    });
