/**
 * lib/__tests__/url-query.spec.ts — useUrlState + codec 单测（T01 验收3，Vue 版）
 *
 * 平移自 React 版（@testing-library/react 挂载），此处改为 @vue/test-utils
 * 挂载宿主组件调用组合式函数，行为契约完全一致：
 * 1. 等于默认值的字段不写入 URL
 * 2. 非法值静默降级为默认值
...
/** 当前用例挂载的宿主组件（afterEach 统一卸载，触发 popstate 监听清理） */
let wrapper: VueWrapper | null = null;

/**
 * 挂载宿主组件并在 setup 中调用 useUrlState，暴露最新的 state / setState
 * （等价 React 版 renderHook，不依赖 vue-router 上下文）。
 */
function setup(initialSearch: string) {
  ...
  const Host = defineComponent({
    setup() {
      ...
      // readonly(reactive) 深度只读类型与可变目标类型不兼容，测试侧统一断言读取
      api.state = state as unknown as TestState;
