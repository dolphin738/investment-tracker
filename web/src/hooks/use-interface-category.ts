/**
 * hooks/use-interface-category.ts — 接口分类（管理员）TanStack Query hooks
 *
 ...
 *   各类写操作并失效分类列表缓存。
 *
 * 与 use-quote-provider.ts 一致的风格：useQuery enabled + useMutation 失效 + toast。
 */
...
export function useInterfaceCategories() {
  ...
  return useQuery<InterfaceCategory[]>({
