/**
 * features/query/dimension-switcher.tsx — 快捷范围常量与解析（阶段 A · A8）
 *
 * 验证点（对齐决策 Q-6 乙 / 清单 27、30）：
 * 1. QUICK_RANGE_OPTIONS 恰为 7 项，顺序与文案锁定（ytd 文案已由「今年至今」→「今年」）
 * 2. resolveQuickRange 对 1w / 1m / 3m / 6m / 1y / ytd / all 七个分支起止日期正确
 * 3. 未知值回落「近1年」（工程师主动偏离：旧实现返回 {}，新实现恒返回具体日期）
 * 4. 返回类型收窄为必填 —— startDate / endDate 恒为非空字符串
 * 5. 不变式：任一预设都满足 startDate <= endDate（含月末回绕等边界日期）
 *
 * 时间控制：resolveQuickRange 内部读 `new Date()`，用 vi.useFakeTimers 钉死系统时间，
 * 避免测试随运行日期漂移。基准日取 2026-06-15（非月末、非闰年 2 月，算术干净）。
 */
...
/** 基准「今天」：2026-06-15 12:00 本地时间（toIsoDate 按本地时区取年月日） */
const BASE_NOW = new Date(2026, 5, 15, 12, 0, 0);

/** 把 Date 转 YYYY-MM-DD（与 lib/constants.toIsoDate 同口径，测试内独立实现避免自证） */
...
describe('resolveQuickRange — 七分支起止日期', () => {
  ...
  it('未知值回落「近1年」（default 分支，与旧 dashboard resolveDateRange 一致）', () => {
  ...
  it('返回类型收窄：所有预设的 startDate / endDate 均为非空字符串（无 undefined）', () => {
...
describe('resolveQuickRange — 边界日期不变式', () => {
  ...
  // 月末 / 闰年 / 年末：验证不会产生 startDate > endDate 的非法区间
  const EDGE_DATES = [
    ...
    (_label, now) => {
      ...
      for (const opt of QUICK_RANGE_OPTIONS) {
        ...
        expect(
          ...
          `${opt.value}: ${r.startDate} > ${r.endDate}`,
        ).toBe(true);
  ...
  it('已知边界（记录当前行为）：月末 setMonth 回绕会使窗口略短于自然月', () => {
    // 2026-03-31 减 1 个月 → JS Date 计算 2026-02-31 → 规范化为 2026-03-03。
    // 这是 Date.setMonth 的固有语义，旧实现（1m/3m）行为一致，非本次回归引入。
    vi.useFakeTimers();
