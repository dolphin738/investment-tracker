/**
 * features/overview/asset-metrics.ts — 概览页 8 指标构造口径单测
 *
 * 覆盖（对应设计 §11.2 N-9 ~ N-14）：
 * 1. 🔴 `formatAmountOrEmpty` 对 `0` / `'0'` 的保护 —— 必须格式化为 ¥0.00，
 *    不得被 falsy 判断吞成「暂无数据」（fusion 分支修复的回归锚点）
 * 2. `null` / `undefined` / `''` → 「暂无数据」
 * 3. 8 项的顺序、key、title、group 分组
 * 4. 「当前总资产」全表只出现 1 次（融合去重的核心验收 N-10）
...
import {
  ...
  type OverviewMetric,
} from '@/features/overview/asset-metrics';

/** 取指定 key 的指标（不存在直接抛，便于定位失败） */
...
describe('buildOverviewMetrics — 结构与顺序', () => {
  ...
  it('标题与设计稿一致，且「当前总资产」只出现 1 次（N-10 去重核心）', () => {
...
describe('buildOverviewMetrics — 数值与空态', () => {
  ...
  it('空入参 → 金额类全部「暂无数据」，比率类回落 formatPercent 的 "-"', () => {
    const metrics = buildOverviewMetrics({});
    ...
    // 比率类保持改造前概览页行为（formatPercent 自带 '-' 兜底）
...
describe('buildOverviewMetrics — 描述文案', () => {
  it('总资产：有截止日 → 「截至 X」；无 → 「数据截止日未知」', () => {
    ...
    expect(
      pick(buildOverviewMetrics({}), 'total-asset').description,
    ).toBe('数据截止日未知');
  });
  ...
  it('总资产：最新快照为手工录入 → 描述追加「· ✋手工」（Q-2 乙）', () => {
    ...
    expect(pick(metrics, 'total-asset').description).toBe(
      '截至 2026-06-15 · ✋手工',
    );
  });
  ...
  it('现金余额：有生效日 → 「生效日 X」；未维护 → 引导文案', () => {
    ...
    expect(
      pick(buildOverviewMetrics({}), 'cash-balance').description,
    ).toContain('未维护');
  });
...
describe('buildOverviewMetrics — trend 方向', () => {
  it('金额类恒 neutral', () => {
    const metrics = buildOverviewMetrics(FULL_INPUT);
    for (const key of ['total-asset', 'market-value', 'cash-balance', 'net-invested']) {
  ...
  it('累计净值：≥1 → up，<1 → down，空 → neutral', () => {
    ...
    expect(
      pick(buildOverviewMetrics({}), 'cumulative-nav').trend,
    ).toBe('neutral');
  });
