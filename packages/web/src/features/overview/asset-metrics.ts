/**
 * features/overview/asset-metrics.ts — 概览页 8 张指标卡的构造口径（纯函数）
 *
 * 【背景】总资产概览由出入金页融合进概览页后（见
 * `docs/designs/overview-fusion-2026-08-06.md`），指标卡由 6 张扩为 8 张。
 * 若继续沿用页面内散装的三元表达式，8 张卡就是 8 套「格式化 + 空态文案 +
 * 涨跌方向」的私有实现，改一处忘一处必然漂移。
 *
 * 【方案】把「原始值 → 展示模型」这条链路收敛到本模块：
 * - {@link formatAmountOrEmpty}：金额格式化 + 空值兜底的**唯一口径**；
 * - {@link buildOverviewMetrics}：按固定顺序产出 8 项 {@link OverviewMetric}。
 *
 * 【为何是纯函数模块（.ts 而非 .tsx）】不依赖 React，测试可直接断言返回值，
 * 无需 render，也就不会被 jsdom / ECharts 的环境问题拖累。
 */
...
/**
 * 金额格式化 + 空值兜底（唯一口径，继承自 `fusion/portfolio-overview-cards` 分支的修复）。
 *
 * 🔴 `null` / `undefined` / `''` → {@link EMPTY_AMOUNT_TEXT}；
 *    但 `0` 与 `'0'` 是**合法金额**，必须照常格式化为 `¥0.00`。
 *    绝不可退回 `value ? format(value) : '暂无数据'` 这种 falsy 判断 ——
 *    那会把「余额恰好为 0」误显示成「暂无数据」（本函数存在的唯一理由）。
 *
 * @param value 原始金额（Decimal 字符串 / number / 空值）
 * @param format 千分位与缩写偏好
 * @returns 已格式化的金额文本，或占位文案
 */
 ...
 *
 * 空值口径：
 * - 金额类走 {@link formatAmountOrEmpty} → 「暂无数据」（0 照常显示）；
 * - 比率类走 `formatPercent` → `'-'`（保持改造前概览页行为，避免回归）。
 *
 * @param input 原始指标值与格式化偏好
 * @returns 8 项展示模型，顺序固定
 */
