/**
 * modules/query/quick-range.ts — 快捷日期范围口径（唯一真相源，决策 Q-6 乙 / INC-01 决策 G）
 *
 * 平移自 React 版 features/query/quick-range.ts，行为契约完全一致。
 *
 * 【为什么单独成文件（T01 增量）】
 * INC-01 要求分析页维度切换组件内嵌的第二套日期范围 UI 改为复用全站唯一控件
 * DateRangeQuickPicker。而 DateRangeQuickPicker 本身要引用 QUICK_RANGE_OPTIONS
 * / resolveQuickRange——若这两者继续留在维度切换组件里，就会形成循环依赖，
 * 且 QUICK_RANGE_OPTIONS 是 const（存在 TDZ 风险，不像函数声明可提升）。
 *
 * 因此把「纯口径」下沉到本叶子模块（无任何组件依赖）：
 * - 维度切换组件与 DateRangeQuickPicker 同时从这里取；
 * - 维度切换组件继续 re-export 全部符号，所有既有 import 路径零改动。
 *
 ...
 * 共用此常量与 resolveQuickRange，不再各自维护本地副本，避免口径漂移。
 */
...
export interface ResolveQuickRangeOptions {
   ...
   *
   * 缺省时回落 ALL_RANGE_FALLBACK_START，保持单参调用的向后兼容。
   */
 ...
 *   opts.allRangeStart = 当前组合 baseDate；未传或组合尚无
 *   首笔买入（baseDate=null）时回落 ALL_RANGE_FALLBACK_START。
 *   startDate 必须显式返回，否则调用方合并时 `?? value.startDate` 会保留
 *   旧起始日，范围不扩大。
 * - 未知值回落「近1年」，与旧 dashboard resolveDateRange 的 default 分支一致。
 *
 * 注意：opts 保持可选 —— 既有单参调用（含 quick-range.test.ts）行为不变。
 */
