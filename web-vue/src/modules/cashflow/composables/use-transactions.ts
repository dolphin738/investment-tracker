/**
 * modules/cashflow/composables/use-transactions.ts — 出入金流水 CRUD vue-query hooks
 *
 * 平移自 React 版 hooks/use-transactions.ts，行为契约一致：
 * - useTransactions：列表 query（分页 + 日期范围 + 类型多选 + 排序）
 ...
 * 决定是否弹「是否同步调整现金余额？[去更新]」；「去更新」只负责切到
 * 「现金余额」页签并打开录入弹窗（派发 CustomEvent，由出入金页监听），
 * 绝不自动修改 CashBalance。
 *
 * 术语约定：本模块面向用户的一律用「出入金」，不再用「交易」指代出入金流水。
 *
...
import { PORTFOLIOS_KEY } from '@/composables/use-portfolios';

/** 出入金流水列表 query key 工厂 */
...
export function useTransactions(
  ...
  return useQuery({
    queryKey: computed(() => {
      ...
      return id
        ? transactionsKey(id, toValue(query))
...
/** 「现金余额」页签聚焦事件名（出入金页监听后切页签并打开录入弹窗） */
export const CASH_BALANCE_FOCUS_EVENT = 'app:focus-cash-balance-input';

/** 派发聚焦「现金余额」录入弹窗事件（软提示「去更新」点击时调用；绝不自动改 CashBalance） */
 ...
 * 为 true 时 toast「是否同步调整现金余额？」+ action「去更新」（切到现金余额页签）；
 * 为 false 时不弹，重算 toast 不受影响（由调用方先展示）。
 *
 * 偏好 store 实例由调用方（setup 内）取得后传入，避免在 mutation 回调里
 * 脱离组件上下文获取 Pinia。
 */
function maybeShowCashHint(
  preferenceStore: ReturnType<typeof usePreferenceStore>,
): void {
  const hintOn = preferenceStore.getPreference('cashHintOnCashflow');
  if (!hintOn) return;
...
function invalidateCashflowRelated(
...
export function useCreateTransaction() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      toast.success(
        `出入金已录入；${buildRecalcSuffix(data?.recalculation, variables.payload.date)}`,
      );
      // FLOW-P0-06：保存出入金后按偏好开关弹软提示（仅在 create/update，delete 无意义）
      maybeShowCashHint(preferenceStore);
...
export function useUpdateTransaction() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      toast.success(
        `出入金已更新；${buildRecalcSuffix(data?.recalculation, variables.payload.date)}`,
      );
      maybeShowCashHint(preferenceStore);
      invalidateCashflowRelated(queryClient);
    },
...
export function useDeleteTransaction() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(
        data?.recalculation
          ? `出入金已删除；${buildRecalcSuffix(data.recalculation)}`
          : '出入金已删除',
      );
      invalidateCashflowRelated(queryClient);
    },
