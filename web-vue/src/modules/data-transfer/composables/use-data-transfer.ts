/**
 * modules/data-transfer/composables/use-data-transfer.ts — 数据导入导出 vue-query hooks
 *
 * 平移自 React 版 hooks/use-data-transfer.ts，行为契约一致：
 * - useExportData：导出 → 触发下载
 * - useDownloadTemplate：下载导入模板
 * - useImportPreview：导入预览（不落库）
 * - useImportCommit：提交 → 成功后 invalidate 六组相关查询（T05 验收 7）
 *
 * 实现说明：React 的 TanStack Query（useMutation）在 Vue 侧对应
 * @tanstack/vue-query 的 useMutation，回调参数形态一致；文件名拼接 / Blob 下载
 * 复用 lib/csv-download.ts（B5 已平移），应用时区日期复用 lib/constants。
 */
...
import { downloadBlob, sanitizeFilename } from '@/lib/csv-download';
