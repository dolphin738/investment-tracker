 ...
 *
 * 方案 B:SecurityTrade 是持仓推导唯一来源,任何写入都会影响持仓 / 净值 / XIRR /
 * 快照 / 概览,故统一失效这些 query 缓存。
 *
 * INC-04 物理并表:费用明细(佣金/印花税/其他)已并入 security_trades 单行,
 * 买卖录入不再单独写费用表,写入只需失效计算链路 query 缓存。
 */
