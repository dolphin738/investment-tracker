/**
 * modules/cash-balance/composables/use-cash-balances.ts — 现金余额 vue-query hooks
 *
 * 平移自 React 版 hooks/use-cash-balances.ts，行为契约一致：
...
export function useCashBalances(
  ...
  query: MaybeRefOrGetter<CashBalanceQuery> = {},
) {
  return useQuery({
    queryKey: computed(() => {
      const id = toValue(portfolioId);
      return id
        ? ['cash-balances', 'list', id, toValue(query)]
...
export function useLatestCashBalance(
  ...
  return useQuery({
    ...
    queryFn: () => getLatestApi(toValue(portfolioId)!),
    enabled: computed(() => Boolean(toValue(portfolioId))),
    staleTime: 30 * 1000,
  });
}
