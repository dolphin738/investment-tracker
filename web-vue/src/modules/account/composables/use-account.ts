/**
 * modules/account/composables/use-account.ts — 账户设置 mutation hooks（改密码 / 改邮箱 / 改资料 / 传头像）
 *
 * 自 React 版 web/src/hooks/use-account.ts 平移，供账户页三个对话框使用。
 *
...
/** 当前用户信息 query key（与 modules/auth/use-auth.ts 的 useProfile 保持一致） */
 ...
 *
 * 成功后清除本地登录态并回到登录页。对应后端 DELETE /api/auth/account（软删除）。
 */
